...
class EtcTypeChecker(varPool: TypeVarPool) extends TypeChecker with EtcBuilder {
  ...
  override def compute(
      typeListener: TypeCheckerListener,
      rootCtx: TypeContext,
      rootEx: EtcExpr
  ): Option[TlaType1] = {
    ...
    try {
      ...
      rootSolver.solve() match {
        case None =>
          onTypeError(
            rootEx.sourceRef,
            s"Error when computing the type of the top expression"
          )
          None
    ...
    ex match {
      ...
      case EtcName(name) =>
        if (ctx.types.contains(name)) {
          ...
          onTypeError(
            ex.sourceRef,
            s"Undefined name $name. Introduce a type annotation."
          )
      ...
      case appEx @ EtcApp(operTypes, args @ _*) =>
        ...
        def onError(types: Seq[TlaType1]): Unit = {
          ...
          if (types.isEmpty) {
            onTypeError(
              appEx.sourceRef,
              s"No matching signature for ${argTypes.length} argument(s)"
            )
            ...
            onTypeError(
              appEx.sourceRef,
              s"Mismatch in argument types. Expected: $sepSigs"
            )
          }
        }
        ...
        solver.addConstraint(
          EqClause(operVar, OperT1(argTypes, resVar))
            .setOnTypeFound(onFound)
            .setOnTypeError(onError)
        )
        // operVar = operType_1 \/ ... \/ operVar = operType_n
        solver.addConstraint(
          OrClause(operTypes.map(EqClause(operVar, _)): _*)
            .setOnTypeError(onError)
      ...
      case EtcAppByName(name, args @ _*) =>
        if (ctx.types.contains(name)) {
          computeRec(
            ctx,
            solver,
            mkApp(ex.sourceRef, Seq(ctx.types(name)), args: _*)
          )
        } else {
          onTypeError(
            ex.sourceRef,
            s"Undefined operator name $name. Introduce a type annotation."
          )
          throw new UnwindException
        }
      ...
      case EtcAbs(scopedEx, binders @ _*) =>
        ...
        val lambdaClause = EqClause(lambdaTypeVar, operType)
          ...
          .setOnTypeError(
            ts =>
              onTypeError(
                ex.sourceRef.asInstanceOf[ExactRef],
      ...
      case EtcLet(name, defEx @ EtcAbs(defBody, binders @ _*), scopedEx) =>
        ...
        def onError(ts: Seq[TlaType1]): Unit = {
          ...
          onTypeError(
            defEx.sourceRef,
            s"Expected $operSig in $name. Found: $sepSigs"
          )
        }
        ...
        val preciseDefType =
          letInSolver.solve() match {
            case None =>
              onTypeError(
                ex.sourceRef,
                s"Error when computing the type of $name"
              )
              throw new UnwindException
      ...
      case EtcLet(_, _, _) =>
        throw new RuntimeException(
          "Bug in type checker. Ill-formed let-expression: " + ex
        )
    }
  }
  ...
  private def translateBinders(
    ...
    binders.zip(setVars.zip(elemVars)).foreach {
      case ((_, setEx), (setVar, elemVar)) =>
        val clause = EqClause(setVar, SetT1(elemVar))
          .setOnTypeFound(onTypeFound(setEx.sourceRef, _))
          .setOnTypeError(
            ts =>
              onTypeError(setEx.sourceRef, "Expected a set. Found: " + ts.head)
          )
        solver.addConstraint(clause)
    }
    // require that these type variables are equal to the computed set types
    binders.zip(setVars.zip(setTypes)).foreach {
      case ((_, setEx), (setVar, setType)) =>
