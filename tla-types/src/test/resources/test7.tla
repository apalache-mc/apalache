...
\* A module inside a module, to avoid importing
...
\* a type definition needed by apalache
IET == [type |-> STRING, heightToVerify |-> Int, hdr |-> BC!SHT]
...
EnvNext ==
    ...
    \/ /\ state = "init"
       ...
       /\ inEvent = NoEvent <: IET
       /\ inEvent' = [type |-> "start", heightToVerify |-> TO_VERIFY_HEIGHT] <: IET
    ...
    \/ /\ state = "working"
       ...
       /\ \E hdr \in BC!SoundSignedHeaders(outEvent.height):
            ...
            inEvent' = [type |-> "responseHeader", hdr |-> hdr] <: IET
...
\* a type definition needed by apalache
OET == [type |-> STRING, height |-> Int, verdict |-> BOOLEAN]

\* a type definition needed by apalache
StackElemT == [ isLeft |-> BOOLEAN, startHeight |-> Int, endHeight |-> Int ]

(* Produce a request event for the pivot of the top element of the requestStack *)
RequestHeaderForTopRequest(pStack) ==
    IF pStack = <<>> <: Seq(StackElemT)
    THEN NoEvent <: OET   \* the stack is empty, no request
    ELSE
      ...
      ELSE \* request for the left branch, the pivot is the endHeight
        [type |-> "requestHeader", height |-> top.endHeight] <: OET

(* When starting the light client *)
OnStart ==
    /\ state = "init"
    /\ inEvent.type = "start"
    /\ inEvent' = NoEvent <: IET \* forget the input event
    /\ state' = "working"
    ...
    /\ IF minTrustedHeight > TRUSTED_HEIGHT
       THEN \* the trusted header is outside of the trusting period
         /\ outEvent' = [type |-> "finished", verdict |-> FALSE] <: OET
       ...
       THEN \* doing upward verification within the trusting period
         LET initStack == << [isLeft |-> TRUE,
                              ...
                              endHeight |-> inEvent.heightToVerify] >> <: Seq(StackElemT)
         IN
         /\ requestStack' = initStack
         /\ outEvent' = RequestHeaderForTopRequest(initStack) <: OET
       ...
       ELSE
         ...
         /\ outEvent' = [type |-> "finished", verdict |-> TRUE] <: OET
         /\ UNCHANGED requestStack
...
\* a type definition for apalache
\*VT == [verdict |-> BOOLEAN, newStack |-> Seq(StackElemT)]
...
OneStepOfBisection(pStoredSignedHeaders) ==
    ...
    THEN [verdict |-> FALSE,
          newStack |-> <<(* empty *)>> <: Seq(StackElemT)] \* TERMINATE immediately
    ELSE \* pass only the header lhdr.header and signed header rhdr
      ...
      ELSE IF lh + 1 = rh \* sequential verification
        ...
        ELSE (*
            ...
            IN
            [verdict |-> TRUE,
             newStack |-> (<<leftReq, rightReq>> <: Seq(StackElemT)) \o Tail(requestStack)]

ET ==
    [ type |-> STRING, verdict |-> BOOLEAN]
...
OnLeftResponseHeader ==
  ...
  /\ LET res == OneStepOfBisection(storedSignedHeaders') IN       \* do one step
      ...
      /\ IF res.newStack = << >> <: Seq(StackElemT)       \* end of the bisection
         THEN /\ outEvent' = [type |-> "finished", verdict |-> res.verdict] <: OET
              /\ state' = "finished"    \* finish with the given verdict
         ELSE /\ outEvent' = RequestHeaderForTopRequest(res.newStack) <: OET
...
NextChanged ==
    Next /\ (~(UNCHANGED bcvars) \/ ~(UNCHANGED lcvars) \/ ~(UNCHANGED envvars))
...
NeverFinish == outEvent.type /= "finished"
...
\* if this property holds after k steps, then it is sufficient to check the client up to k steps
...
\* Last modified Mon Dec 02 23:28:49 CET 2019 by igor
