...
class TestTypeUnifier
  ...
  test("unifying monotypes") {
    assert(
      unifier
        .unify(Substitution.empty, ConstT1("F"), ConstT1("F"))
        .contains((Substitution.empty, ConstT1("F")))
    )
    assert(
      unifier
        .unify(Substitution.empty, IntT1(), IntT1())
        .contains((Substitution.empty, IntT1()))
    )
    assert(
      unifier
        .unify(Substitution.empty, BoolT1(), BoolT1())
        .contains((Substitution.empty, BoolT1()))
    )
    assert(
      unifier
        .unify(Substitution.empty, StrT1(), StrT1())
        .contains((Substitution.empty, StrT1()))
    )
    assert(
      unifier
        .unify(Substitution.empty, RealT1(), RealT1())
        .contains((Substitution.empty, RealT1()))
    )
    val intToInt = parser("Int -> Int")
    assert(
      unifier
        .unify(Substitution.empty, intToInt, intToInt)
        .contains((Substitution.empty, intToInt))
    )
    ...
    assert(
      unifier
        .unify(Substitution.empty, tup1, tup1)
        .contains((Substitution.empty, tup1))
    )
    ...
    assert(
      unifier
        .unify(Substitution.empty, sparse1, sparse2)
        .contains((Substitution.empty, sparse3))
    )
    assert(
      unifier
        .unify(Substitution.empty, tup1, sparse4)
        .contains((Substitution.empty, tup1))
    )
    assert(
      unifier
        .unify(Substitution.empty, sparse4, tup1)
        .contains((Substitution.empty, tup1))
    )
    ...
    assert(
      unifier
        .unify(Substitution.empty, rec1, rec2)
        .contains((Substitution.empty, rec3))
    )
  }
  ...
  test("non-unifying monotypes") {
    ...
    assert(
      unifier.unify(Substitution.empty, ConstT1("F"), ConstT1("G")).isEmpty
    )
    ...
    assert(
      unifier
        .unify(Substitution.empty, intAndBoolToInt, boolAndIntToInt)
        .isEmpty
    )
    val tup1 = parser("<<Int, Bool>>")
    ...
    assert(
      unifier
        .unify(
          Substitution.empty,
          FunT1(VarT1(0), IntT1()),
          FunT1(BoolT1(), VarT1(1))
        )
        .contains(
          (Substitution(0 -> BoolT1(), 1 -> IntT1()), FunT1(BoolT1(), IntT1()))
        )
    )
    assert(
      unifier
  ...
  test("non-unifying polytypes") {
    ...
    assert(
      unifier.unify(Substitution.empty, parser("a"), parser("Set(a)")).isEmpty
    )
  }

  test("unifying with transitivity") {
    val expectedSubstitution =
      Substitution(1 -> parser("PERSON"), 2 -> parser("PERSON"))
    assert(
      unifier
        .unify(
          Substitution.empty,
          parser("Set(b) -> Set(b)"),
          parser("Set(c) -> Set(PERSON)")
        )
  ...
  test("cycle detection") {
    ...
    assert(
      unifier
        .unify(
          Substitution(0 -> VarT1("b"), 1 -> VarT1("a")),
          VarT1("a"),
          VarT1("b")
        )
        .isEmpty
