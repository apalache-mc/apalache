...
//  }
  ...
  def getVars(ex: TlaEx): Set[Var] = ex match {
    case OperEx(TlaActionOper.prime, NameEx(n)) => Set(n)
    case OperEx(_, args @ _*) => { args map getVars }.foldLeft(Set.empty[Var]) {
      _ ++ _
    }
    case _ => Set.empty[Var]
  }
...
sealed case class Exists(ex: OperEx, boundVar: Star, set: Star, body: AlphaEx)
    extends AlphaEx(ex)

sealed case class ITE(ex: OperEx, IF: Star, THEN: AlphaEx, ELSE: AlphaEx)
    extends AlphaEx(ex)
...
object AlphaTransform {
  def apply(ex: TlaEx): AlphaEx = ex match {
    ...
    case e @ OperEx(
          TlaSetOper.in,
          OperEx(TlaActionOper.prime, NameEx(v)),
          rhs
        ) =>
      AsgnEx(e, v, Star(rhs))
