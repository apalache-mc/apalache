...
object CoverData {
  ...
  sealed case class ProblemData(
      problemUIDs: IncompleteCover, blameMap: Map[UID, IncompleteCover]
  ) {
  ...
  def uncoveredBranchPoints(varName: String)(cd: CoverData): ProblemData = cd match {
    ...
    case BranchPoint(loc, branches @ _*) =>
      ...
      val branchIssues =
        branches.foldLeft(ProblemData(NoProblem(), Map.empty)) { case (pd, brCd) =>
    ...
    case NonBranch(loc, elements @ _*) =>
      ...
      if (elemIssues.exists(_.problemUIDs.isEmpty)) {
        ...
        val problemAggregate = elemIssues.foldLeft(ProblemData(NoProblem(), Map.empty)) { case (pd, brPd) =>
          ProblemData(
