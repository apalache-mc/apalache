...
sealed case class BranchPoint(uid: UID, branches: CoverData*)
    extends CoverData(uid)
sealed case class NonBranch(uid: UID, elements: CoverData*)
    extends CoverData(uid)
...
object CoverData {
  ...
  def uncoveredBranchPoints(varName: String)(cd: CoverData): ProblemData =
    cd match {
      ...
      case NonBranch(loc, elements @ _*) =>
        ...
        if (elemIssues.exists(_.problemUIDs.isEmpty)) {
          ...
          val problemAggregate =
            elemIssues.foldLeft(ProblemData(NoProblem(), Map.empty)) {
              case (pd, brPd) =>
                ProblemData(
