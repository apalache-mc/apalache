...
class CoverChecker(
    ...
    manuallyAssigned: Set[String] = Set.empty
) {
  ...
  private def mkCoverInternal(
    ...
    case OperEx(oper, args @ _*)
      ...
      if (oper == TlaBoolOper.and)
        NonBranch(ex.ID, processedChildArgs: _*)
      else
    ...
    case OperEx(TlaOper.eq, OperEx(TlaActionOper.prime, NameEx(name)), star) =>
      ...
      if (!manuallyAssigned.contains(name)) Candidate(name, ex.ID)
      else NonCandidate(ex.ID)
    case OperEx(
        BmcOper.assign,
  ...
  def coveredVars(ex: TlaEx): Set[String] = {
    ...
    val potentialProblemMap = (allVariables map { v =>
      v -> CoverData.uncoveredBranchPoints(v)(cd)
    }).toMap

    potentialProblemMap.keySet.filter { k =>
      potentialProblemMap(k).noProblem
    }
