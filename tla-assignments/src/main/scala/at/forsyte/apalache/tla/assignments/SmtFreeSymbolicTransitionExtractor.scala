...
import at.forsyte.apalache.tla.lir.storage.{
  BodyMap,
  BodyMapFactory,
  SourceLocator
}
...
class SmtFreeSymbolicTransitionExtractor(
  ...
  private def getLocString(ex: TlaEx) =
    sourceLoc.sourceOf(ex).getOrElse("<[UNKNOWN]>")

  /** Checks whether expressions, which cannot contain assignments, use unassigned variables */
  private def throwOnUseBeforeAssignment(
    ...
    case ex @ OperEx(
          BmcOper.assign,
          OperEx(TlaActionOper.prime, NameEx(_)),
          _
        ) =>
    ...
    case ex @ OperEx(TlaActionOper.prime, NameEx(name))
        if unassignedVars.contains(name) =>
      val locString = getLocString(ex)
      // TODO: Update manual on assignments, see #368
      throw new AssignmentException(
    ...
    case LetInEx(body, defs @ _*) =>
      defs foreach { d =>
        throwOnUseBeforeAssignment(unassignedVars)(d.body)
      }
  ...
  private case class PartialState(
      unassignedVars: Set[String],
      partialStrat: StrategyType
  )

  // Since disjunction and ITE behave exactly the same w.r.t. assignments, we write a unified routine
  private def handleDisjunctionOrITE(
      s: PartialState,
      letInOperMap: BodyMap,
      args: Seq[TlaEx]
  ): PartialState = {
    ...
    val unassignedEverywhere = unassignedVarsSeq.foldLeft(s.unassignedVars) {
      _.intersect(_)
    }
    val unassignedSomewhere = unassignedVarsSeq.foldLeft(Set.empty[String]) {
      _.union(_)
    }
    ...
    val unifiedStrat =
      (childStates map { _.partialStrat }).foldLeft(s.partialStrat) {
  ...
  private def getStrategyOptInternal(
    ...
    case ex @ OperEx(
      ...
      if (currentState.unassignedVars.contains(name))
        PartialState(
          currentState.unassignedVars - name,
          currentState.partialStrat :+ ex.ID
        )
      else currentState
    ...
    case OperEx(TlaBoolOper.exists, NameEx(_), assignmentFreeSetEx, subEx) =>
      ...
      throwOnUseBeforeAssignment(currentState.unassignedVars)(
        assignmentFreeSetEx
      )
    ...
    case LetInEx(body, defs @ _*) =>
      ...
      assert(defs.forall { d =>
        d.formalParams.isEmpty && !d.isRecursive
      })
    ...
    case OperEx(TlaOper.apply, NameEx(operName)) =>
      ...
      val newStateOpt = knownBodyOpt map {
        getStrategyOptInternal(currentState, letInOperMap)(_)
      }
  ...
  def getStrategy(vars: Set[String], actionEx: TlaEx): StrategyType = {
    ...
    val finalState =
      getStrategyOptInternal(PartialState(vars, Seq.empty), Map.empty)(actionEx)
    ...
    if (missingVars.nonEmpty)
      throw new AssignmentException(
        s"No assignments found for: ${missingVars.mkString(", ")}"
      )
...
object SmtFreeSymbolicTransitionExtractor {
  val MANUAL_LINK =
    "https://apalache.informal.systems/docs/apalache/principles.html#assignments"
