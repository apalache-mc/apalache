...
class SymbTransGenerator(tracker: TransformationTracker) {
  ...
  private[assignments] object helperFunctions {
    ...
    def allCombinations[ValType](
        p_sets: Seq[Set[Set[ValType]]]
    ): Set[Set[ValType]] = {
      if (p_sets.isEmpty)
        ...
        (for { s <- one } yield rest.map(st => st ++ s))
          .fold(Set.empty[Set[ValType]])(_ ++ _)
      }
    }

    def labelsAt(p_ex: TlaEx, p_partialSel: SelMapType): Set[UID] =
      labelsAt(p_ex.ID, p_partialSel)
    ...
    def allSelections(
        ex: TlaEx,
        letInMap: letInMapType = Map.empty
    ): SelMapType = ex match {
      ...
      case OperEx(TlaBoolOper.and, args @ _*) =>
        ...
        val unifiedMap = (args map { allSelections(_, letInMap) })
          .fold(Map.empty[UID, AssignmentSelections]) { _ ++ _ }
        ...
        if (mySet.isEmpty || mySet.exists(_.isEmpty)) unifiedMap
        else unifiedMap + (ex.ID -> mySet)
      ...
      case OperEx(TlaControlOper.ifThenElse, _, thenAndElse @ _*) =>
        val unifiedMap = (thenAndElse map { allSelections(_, letInMap) })
          .fold(Map.empty[UID, AssignmentSelections]) { _ ++ _ }
...
//        newArgs.headOption.map( sliceWith( selection, allSelections) ).getOrElse( ex )
        newArgs match {
          ...
          case _ =>
            OperEx(
              TlaBoolOper.or,
              newArgs map sliceWith(selection, allSelections): _*
            )
        }
      ...
      case ex @ OperEx(TlaControlOper.ifThenElse, ifEx, args @ _*) =>
        val newTail = args.map(
          x =>
            if (labelsAt(x, allSelections) exists {
                  selection.contains
                })
  ...
  def apply(phi: TlaEx, asgnStrategy: StrategyType): Seq[SymbTrans] = {
    ...
    val newStrat = asgnStrategy map { x =>
      asgnTransform.getReplacements.getOrElse(x, x)
    }
