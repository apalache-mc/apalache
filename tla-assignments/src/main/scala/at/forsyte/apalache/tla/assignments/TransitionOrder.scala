...
class TransitionOrder(sourceLocator: SourceLocator) extends LazyLogging {
  ...
  /**
    ...
    */
  private def seqLocLT(
      a: Seq[SourceLocation],
      b: Seq[SourceLocation]
  ): Boolean =
    ...
    def findLoc(uid: UID): SourceLocation = {
      sourceLocator.sourceOf(uid) match {
        ...
        case None      =>
          ...
          SourceLocation(
            "unknown",
            new SourceRegion(SourcePosition(1), SourcePosition(2))
          )
      }
    }
  ...
  private def transLT(a: SymbTrans, b: SymbTrans): Boolean =
    seqLocLT(getSortedLocs(a), getSortedLocs(b))
}
