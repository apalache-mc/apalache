...
class TestSymbTransPass extends FunSuite with TestingPredefs {
  ...
  def testFromDecls(
    ...
    SymbolicTransitionExtractor(tracker)(
      vars,
      preprocessed.operDeclarations.find(_.name == p_next).get.body
    )
  }
...
//    saveWriter.close()
  ...
  test("Test labelsAt") {
    ...
    assert(
      gen.helperFunctions
        .labelsAt(NullEx, Map.empty[UID, Set[Set[UID]]])
        .isEmpty
    )
  }
  ...
  test("Test no strat") {
    ...
    val decls =
      Seq(TlaOperDecl("Next", List(), next), TlaVarDecl("x"), TlaVarDecl("z"))
