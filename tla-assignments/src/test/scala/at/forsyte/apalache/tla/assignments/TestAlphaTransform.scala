...
class TestAlphaTransform extends FunSuite with TestingPredefs {
  ...
  def specFromFile(p_file: String, p_next: String = "Next"): TlaEx = {
    ...
    val inlined =
      ModuleByExTransformer(InlinerOfUserOper(bodyMap, tracker))(uniqueVarDecls)
    val explLetIn = ModuleByExTransformer(
      LetInExpander(tracker, keepNullary = false)
    )(inlined)
    val afterDesugarer = ModuleByExTransformer(Desugarer(tracker))(explLetIn)
    val preprocessed =
      ModuleByExTransformer(PrimedEqualityToMembership(tracker))(afterDesugarer)
  ...
  def correctRecursiveApplication(exs: Traversable[TlaEx]): Boolean = {
    val trs = (exs map { ex =>
      ex -> AlphaTransform(ex)
    }).toMap
    def argMatch(
        exargs: Traversable[TlaEx],
        args: Traversable[AlphaEx]
    ): Boolean =
      (exargs map { arg =>
        trs.getOrElse(arg, AlphaTransform(arg))
      }) == args

    trs forall {
      case (ex @ OperEx(_, _*), tr) =>
        tr match {
          case AndOr(OperEx(_, exargs @ _*), args @ _*) =>
            argMatch(exargs, args)
          case ITE(OperEx(_, exargs @ _*), i, t, e) =>
            argMatch(exargs, Seq(i, t, e))
          case Exists(OperEx(_, exargs @ _*), x, s, b) =>
            argMatch(exargs, Seq(x, s, b))
          case AsgnEx(OperEx(_, exargs @ _*), v, r) =>
            argMatch(exargs.tail, Seq(r)) // skip variable
          case Star(e) => e == ex
          case _       => false
        }
