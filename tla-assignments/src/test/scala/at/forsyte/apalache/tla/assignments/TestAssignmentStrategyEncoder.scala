...
class TestAssignmentStrategyEncoder extends FunSuite with TestingPredefs {
  ...
  val vars = Set("x", "y")
  val defaultVarsDelta = (vars map { x =>
    x -> False()
  }).toMap
  ...
  test("Test staticAnalysis: or") {
    ...
    assertEqualSAD(
      ...
      StaticAnalysisData(
        ...
        Map(
          "x" -> And(Variable(asgnX1.ID.id), Variable(asgnX2.ID.id)),
          "y" -> And(False(), False())
        ),
        Map(
          asgnX1.ID.id -> Set.empty[String],
          asgnX2.ID.id -> Set.empty[String]
        ),
    ...
    assertEqualSAD(
      ...
      StaticAnalysisData(
        ...
        Set.empty,
        Map(
          "x" -> And(Variable(asgnX1.ID.id), False()),
          "y" -> And(False(), False())
        ),
  ...
  test("Test staticAnalysis: and") {
    ...
    assertEqualSAD(
      ...
      StaticAnalysisData(
        ...
        Set(
          (asgnX.ID.id, asgnX.ID.id),
          (asgnY.ID.id, asgnY.ID.id),
          (asgnX.ID.id, asgnY.ID.id),
          (asgnY.ID.id, asgnX.ID.id)
        ),
        Set.empty,
        Map(
          "x" -> Or(Variable(asgnX.ID.id), False()),
          "y" -> Or(False(), Variable(asgnY.ID.id))
        ),
    ...
    assert(
      andAsgnsSAD.simplified.delta == Map(
        "x" -> Variable(asgnX.ID.id),
        "y" -> Variable(asgnY.ID.id)
      )
    )
    ...
    assert(
      andMixed1SAD.simplified.delta == Map(
        "x" -> Variable(asgnX.ID.id),
        "y" -> False()
      )
    )
  ...
  test("Test staticAnalysis: exists") {
    ...
    val existsDoubleAsgn =
      exists(n_z, enumSet(prime(n_y)), exists(n_t, emptySet(), asgn))
    val existsCmp = exists(n_z, enumSet(prime(n_y)), cmp)
    val existsDoubleCmp =
      exists(n_z, enumSet(prime(n_y)), exists(n_t, emptySet(), cmp))
  ...
  test("Test staticAnalysis: LET-IN") {
    ...
    assertEqualSAD(
      ...
      StaticAnalysisData(
        ...
        Set(
          (asgnX.ID.id, asgnX.ID.id),
          (asgnY1.ID.id, asgnY1.ID.id),
          (asgnY2.ID.id, asgnY2.ID.id),
          (asgnX.ID.id, asgnY1.ID.id),
          (asgnY1.ID.id, asgnX.ID.id),
          (asgnX.ID.id, asgnY2.ID.id),
          (asgnY2.ID.id, asgnX.ID.id)
        ),
        ...
        Map(
          asgnX.ID.id -> Set.empty[String],
          asgnY1.ID.id -> Set.empty[String],
          asgnY2.ID.id -> Set.empty[String]
        ),
        Map(
          asgnX.ID.id -> asgnX,
          asgnY1.ID.id -> asgnY1,
