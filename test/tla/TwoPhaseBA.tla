...
PRED_P1 ==
    [s \in {"init", "committed", "aborted"} |->
        tmState = s
    ]

PRED_P2 ==
    [rm \in RM |->
        AsMsg([type |-> "Prepared", rm |-> rm]) \in msgs
    ]
...
PRED_P5 ==
    AsMsg([type |-> "Commit"]) \in msgs

PRED_P6 ==
    AsMsg([type |-> "Abort"]) \in msgs

PRED_P7 ==
    [rm \in RM, s \in {"working", "prepared", "committed", "aborted"} |->
        rmState[rm] = s
    ]
