...
(*
  ...
  It is an abstraction of TwoPhase.

  Igor Konnov, 2020
 *)

CONSTANT RM \* The set of resource managers
    (* In this case, it is a uninterpreted sort *)

\* this annotation will go away in the future
a <: b == a    

ASSUME(RM <: "Set('sortRM)")
...
(*
\* tla does not allow that

ASSUME(rmWorking <: "'sortRM -> Bool")  
ASSUME(rmPrepared <: "'sortRM -> Bool")  
ASSUME(rmCommitted <: "'sortRM -> Bool")  
ASSUME(rmAborted <: "'sortRM -> Bool")  
ASSUME(sentPrepared <: "'sortRM -> Bool")  
ASSUME(tmState <: "'sortTM")
*)
...
Init ==   
  ...
  (*************************************************************************)
  /\ \A rm \in RM: rmWorking[rm] = TRUE
  /\ \A rm \in RM: rmPrepared[rm] = FALSE
  /\ \A rm \in RM: rmCommitted[rm] = FALSE
  /\ \A rm \in RM: rmAborted[rm] = FALSE
  /\ \A rm \in RM: sentPrepared[rm] = FALSE
  /\ tmState = "init"
  (*
  /\ tmPrepared   = {} <: {STRING}
  /\ msgs = ({} <: {MT})
...
TCConsistent ==  
  ...
  (*************************************************************************)
  \A rm1, rm2 \in RM: ~(rmAborted[rm1] /\ rmCommitted[rm2])
  (*
