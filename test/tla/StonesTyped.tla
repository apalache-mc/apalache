------------------------- MODULE StonesTyped -------------------------------
\* THIS IS A TYPED VERSION OF Stones
\* The original is avaiable at: 
\* https://github.com/tlaplus/Examples/tree/master/specifications/Stones
...
(***************************************************************************)
EXTENDS Integers, Sequences, FiniteSets, TLC, Typing 
...
Partitions(seq, wt) == "(Seq(Int), Int) => Set(Seq(Int))" ##
  IF Len(seq) = N
    THEN {seq}
...
(***************************************************************************)
Weighs(seq, wt) == "(Seq(Int), Int) => Bool" ##
  \E coef \in [1..N -> -1..1] : 
