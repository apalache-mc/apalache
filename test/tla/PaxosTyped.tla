------------------------- MODULE PaxosTyped -------------------------------
\* THIS IS A TYPED VERSION OF Paxos
...
TypeAssumptions1 ==
    /\ AssumeType(Value, "Set(VALUE)")
    /\ AssumeType(Acceptor, "Set(PROC)")
    /\ AssumeType(Quorum, "Set(Set(PROC))")
...
\* the type checker needs a hint, as it cannot find a type
\* of unbounded CHOOSE
None == LET NoneValue == "VALUE" :>
            CHOOSE v : v \notin Value
        IN
        NoneValue
  (*************************************************************************)
...
TypeAssumptions2 ==
    /\ AssumeType(maxBal, "PROC -> Int")
    /\ AssumeType(maxVBal, "PROC -> Int")
    /\ AssumeType(maxVal, "PROC -> VALUE")
    /\ AssumeType(msgs,
        "Set([type: Str, bal: Int, acc: PROC, mbal: Int, mval: VALUE, val: VALUE])")
