...
class TestUnroller
  ...
  private var unroller = new Unroller(
    new UniqueNameGenerator,
    noTracker,
    new IncrementalRenaming(noTracker)
  )

  override def beforeEach(): Unit = {
    unroller = new Unroller(
      new UniqueNameGenerator,
      noTracker,
      new IncrementalRenaming(noTracker)
    )
  ...
  def exAsDecl(pa: (String, TlaEx)): TlaOperDecl =
    TlaOperDecl(pa._1, List.empty, pa._2)
      ...
      case d @ TlaOperDecl(_, _, body) =>
        !d.isRecursive &&
          (body match {
            case LetInEx(
                paramNormalBody,
                TlaOperDecl(uniqueName, Nil, NameEx("p"))
                ) =>
              paramNormalBody match {
                case LetInEx(
                    ...
                    TlaOperDecl(
                      _,
                      Nil,
                      OperEx(TlaOper.apply, NameEx(`uniqueName`))
                    )
                    ) =>
                  true
                case _ => false
