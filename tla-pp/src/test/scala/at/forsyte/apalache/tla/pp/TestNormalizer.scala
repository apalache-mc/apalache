...
class TestNormalizer extends AnyFunSuite with BeforeAndAfterEach {
  ...
  test("""~ \E x \in s . y ~~> \A x \in s . ~y""") {
    ...
    val expected =
      tla
        .forall(tla.name("x").as(BoolT1), tla.name("s").as(SetT1(IntT1)), tla.not(tla.name("y").as(BoolT1)).as(BoolT1))
        .as(BoolT1)
    assert(expected == output)
  }
  ...
  test("""~ \A x \in s . y ~~> \E x \in s . ~y""") {
    ...
    val expected =
      tla
        .exists(tla.name("x").as(BoolT1), tla.name("s").as(SetT1(IntT1)), tla.not(tla.name("y").as(BoolT1)).as(BoolT1))
        .as(BoolT1)
    assert(expected == output)
  }
