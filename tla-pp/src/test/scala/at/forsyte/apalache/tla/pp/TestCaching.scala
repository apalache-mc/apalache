...
class TestCaching extends FunSuite with BeforeAndAfterEach with TestingPredefs {
  private var cacher =
    new Cacher(new UniqueNameGenerator, TrackerWithListeners())
  ...
  test("Single expression") {
    ...
    val cached =
      cacher.cacheApplicaitons(Set(operName), Cacher.DecisionFns.recursive)(
        decl
      )
  ...
  test("Nested") {
    ...
    val decl = TlaOperDecl(
      operName,
      List("p", "q"),
      tla.appOp(n_A, tla.appOp(n_A, 0, "p"), "q")
    )
  ...
  test("Inner recursive LET-IN") {
    ...
    val cached =
      cacher.cacheApplicaitons(operNames, Cacher.DecisionFns.recursive)(decl)
    assert(cached.isInstanceOf[TlaOperDecl])
    val asOper = cached.asInstanceOf[TlaOperDecl]
    assert(!asOper.isRecursive)
          ...
          declB1.body == tla.appOp(n_B, 1) && (
            body match {
              case OperEx(
                  TlaArithOper.plus,
                  `one`,
                  LetInEx(letInBody, defs @ _*)
                  ) =>
                (defs exists { declT0 =>
                  (declT0.body == tla.appOp(n_T, 0)) &&
                  letInBody == tla
                    .plus(tla.appDecl(declT0), tla.appDecl(declB1))
                }) &&
                  (
                    defs exists { declT =>
                      declT.body match {
