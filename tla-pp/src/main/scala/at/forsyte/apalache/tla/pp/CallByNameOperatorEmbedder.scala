...
/**
 * Replaces instances of call-by-name, identified by the ApalacheOper.callByName wrapper, with a LET-expression,
 * embedding the operator definition.
 *
 * The purpose of this is to allow for a) the deletion of A from the operator declaration set b) the removal of a
 * BodyMap dependency in FoldX rewriting. * Example: `A(p,q) == p + q`, `CallByName(A) ` becomes `CallByName(LET
 * localA(p,q) == p + q IN localA)`
 *
 * @author
 *   Jure Kukovec
 */
