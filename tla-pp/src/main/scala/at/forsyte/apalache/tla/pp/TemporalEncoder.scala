 ...
 */
@Singleton
class TemporalEncoder(gen: UniqueNameGenerator, stateVariables: Set[String], tracker: TransformationTracker)
...
object TemporalEncoder {
  def apply(gen: UniqueNameGenerator, stateVariables: Set[String], tracker: TransformationTracker): TemporalEncoder = {
    new TemporalEncoder(gen, stateVariables, tracker)
  }
}
