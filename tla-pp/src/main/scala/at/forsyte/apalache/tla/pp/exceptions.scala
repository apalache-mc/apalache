...
/**
 * An exception that should be thrown when preprocessing steps resulted in an irrecoverable error. This exception should
 * be treated as a bug, so a stack trace is expected.
 *
 * @param message
 *   the error message
 */
...
/**
 * An exception that should be thrown when the TLA+ code is not following the Apalache guidelines, e.g., missing
 * annotations for the recursive operators. This exception should be treated as the input error: no stack trace, normal
 * messages.
 *
 * @param message
 *   error message
 * @param sourceId
 *   optional id of the expression (or declaration) that caused the error
 */
