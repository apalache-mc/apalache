...
class ExprOptimizer(
  ...
  override val partialTransformers =
    List(transformFuns, transformSets, transformCard, transformExistsOverSets)
      ...
      found match {
        ...
        case _ =>
          throw new IllegalArgumentException(
            s"Access to non-existent record field $accessedKey"
          )
      }
  }
  ...
  private def transformCard: PartialFunction[TlaEx, TlaEx] = {
    case OperEx(
        TlaOper.eq,
        OperEx(TlaFiniteSetOper.cardinality, set),
        ValEx(TlaInt(intVal))
        ) if intVal == BigInt(0) =>
    ...
    case OperEx(
        TlaArithOper.gt,
        card @ OperEx(TlaFiniteSetOper.cardinality, _),
        ValEx(TlaInt(intVal))
        ) if intVal.isValidInt =>
  ...
  private def transformExistsOverSets: PartialFunction[TlaEx, TlaEx] = {
    case OperEx(
        TlaBoolOper.exists,
        NameEx(x),
        OperEx(TlaSetOper.filter, NameEx(y), s, e),
        g
        ) =>
      // \E x \in {y \in S: e}: g becomes \E y \in S: e /\ g [x replaced with y]
      val newPred =
        ReplaceFixed(NameEx(x), NameEx(y), tracker).apply(tla.and(e, g))
      val result = tla.exists(tla.name(y), s, newPred)
      transformExistsOverSets.applyOrElse(result, { _: TlaEx =>
        result
      }) // apply recursively to the result
    ...
    case OperEx(
      ...
      val newPred =
        ReplaceFixed(NameEx(boundVar), tla.appOp(NameEx(letName)), tracker)
          .apply(pred)
      ...
      def mkExistsRec(name: String, set: TlaEx, pred: TlaEx): TlaEx = {
        ...
        transformExistsOverSets.applyOrElse(exists, { _: TlaEx =>
          exists
        }) // apply recursively, to optimize more
      }

      pairs.foldLeft(letIn) {
        case (exprBelow, (name, set)) => mkExistsRec(name, set, exprBelow)
      }
