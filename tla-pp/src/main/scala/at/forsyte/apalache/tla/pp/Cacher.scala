...
class Cacher(nameGenerator: UniqueNameGenerator, tracker: TransformationTracker)
    extends TlaModuleTransformation {

  def prepareAppMap(
      operNames: Set[String],
      init: Map[TlaEx, String] = Map.empty
  ): TlaEx => Map[TlaEx, String] = {
    def inner(
  ...
  def replaceApplicationsWithNullary(
    ...
    case ex @ OperEx(TlaOper.apply, opName @ NameEx(operName), args @ _*)
        if operNames.contains(operName) =>
      ...
      appMap
        ...
        .getOrElse {
          val newArgs = args map replaceApplicationsWithNullary(
            operNames,
            appMap,
            letInDecisionFn
          )
          if (args == newArgs) ex
          else OperEx(TlaOper.apply, opName +: newArgs: _*)
        }
    ...
    case ex @ LetInEx(body, defs @ _*) =>
      ...
      val extendedOperNames = operNames ++ defs
        .withFilter(letInDecisionFn)
        .map(_.name)
      val modifiedDefs = defs map {
        cacheApplicaitons(extendedOperNames, letInDecisionFn)(_)
      ...
      val extendedDefs = diffMap.toSeq map {
        case (k, v) =>
          TlaOperDecl(
            ...
            List.empty,
            fixPointRepeat(
              replaceApplicationsWithNullary(
                extendedOperNames,
                diffMap - k,
                letInDecisionFn
              ),
      ...
      val newBody = replaceApplicationsWithNullary(
        extendedOperNames,
        extendedAppMap,
        letInDecisionFn
      )(body)
