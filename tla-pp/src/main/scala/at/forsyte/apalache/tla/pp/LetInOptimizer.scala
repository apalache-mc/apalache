...
class LetInOptimizer(tracker: TransformationTracker) extends TlaExTouchTransformation {
  ...
  override def apply(input: KeepOrTouchEx): KeepOrTouchEx = {
    unusedNames = Set.empty
    transform(input)
  }

  private def transform: TlaExTouchTransformation = tracker.trackTouchEx {
    _.flatMap {
      case ex @ ValEx(_) =>
        keep(ex)
      ...
      case ex @ OperEx(oper, args @ _*) =>
        val trArgs = args.map(a => transform(touch(a)))
        if (trArgs.forall(_.isLeft)) {
          // No argument has changed.
          keep(ex)
        } else {
          // At least one argument has changed: Produce a new expression and tag it as to be touched.
          touch(OperEx(oper, trArgs.map(a => a.fold(e1 => e1, e2 => e2)): _*)(ex.typeTag))
        }
