...
class Normalizer(tracker: TransformationTracker) extends TlaExTransformation {
  ...
  private def nnf(neg: Boolean): TlaExTransformation = tracker.track {
    ...
    case OperEx(TlaSetOper.subseteq, left, right) =>
      OperEx(
        if (neg) TlaSetOper.supsetProper else TlaSetOper.subseteq,
        transform(left),
        transform(right)
      )
    ...
    case OperEx(TlaSetOper.subsetProper, left, right) =>
      OperEx(
        if (neg) TlaSetOper.supseteq else TlaSetOper.subsetProper,
        transform(left),
        transform(right)
      )
    ...
    case OperEx(TlaSetOper.supsetProper, left, right) =>
      OperEx(
        if (neg) TlaSetOper.subseteq else TlaSetOper.supsetProper,
        transform(left),
        transform(right)
      )
    ...
    case OperEx(TlaSetOper.supseteq, left, right) =>
      OperEx(
        if (neg) TlaSetOper.subsetProper else TlaSetOper.supseteq,
        transform(left),
        transform(right)
      )
  ...
  private def nnfLetIn(
      neg: Boolean,
      body: TlaEx,
      defs: Seq[TlaOperDecl]
  ): TlaEx = {
    ...
    val negReplacedBody = replacements.foldLeft(newBody) {
      case (b, tr) =>
        tr(b)
    }

    val negDefs = defs withFilter { d =>
      negOpers.contains(d.name)
    } map { d =>
