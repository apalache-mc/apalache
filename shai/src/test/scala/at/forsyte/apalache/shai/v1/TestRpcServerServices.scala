...
object RpcServerSpec extends DefaultRunnableSpec {
  val tests = CmdExecutorTestCases.tests ++ TransExplorerTestCases.tests
  ...
  def spec = suite("RpcServerSpec")(tests: _*)
    ...
    .provideSomeLayerShared[ZEnv](
        RpcServer.createCmdExecutorService.toLayer ++ RpcServer.createTransExplorerService.toLayer)
}
