...
class SaveToStoreTracker(sourceStore: SourceStore) extends TransformationTracker {
   ...
   */
  override def trackEx(tr: TlaExTransformation): TlaExTransformation = { input: TlaEx =>
    val output = tr(input)
   ...
   */
  override def trackDecl(tr: TlaDeclTransformation): TlaDeclTransformation = { input: TlaDecl =>
    val output = tr(input)
    if (output.ID != input.ID) {
      sourceStore.findOrWarn(input.ID).foreach { loc => sourceStore.add(output.ID, loc) }
    }
   ...
   * @return a new declaration transformation that applies tr and tracks the changes
   */
  override def trackOperDecl(tr: TlaOperDecl => TlaOperDecl): TlaOperDecl => TlaOperDecl = { input: TlaOperDecl =>
    val output = tr(input)
    if (output.ID != input.ID) {
      sourceStore.findOrWarn(input.ID).foreach { loc => sourceStore.add(output.ID, loc) }
    }
