...
/**
 * <p>This class maps expression identifiers to regions of source. It maintains the following invariant: If e2 is a
 * subexpression of e1, then region(e2.id) is inside of region(e1.id). This class assumes that the TLA+ expressions have
 * been assigned identifiers.</p>
 *
 * <p>This is the least wasteful implementation that came to my mind. More importantly, it is designed in such a way
 * that there are no gaps in the source code information, that is, if we store the source location of an expression,
 * then we also store the source location of every its subexpression.</p>
 *
 * @author
 *   Igor Konnov
 */
