...
/**
 * We have to define a package object in its own package, not `at.forsyte.apalache.io.annotations`. Otherwise, scala
 * plugin goes nuts. To get all tha package definitions, import this package as:
 *
...
package object store {
  ...
  /**
   * A mapping from unique identifiers to annotations, e.g., from operator identifiers to annotations. This mapping is
   * mutable.
   */
   ...
   *   annotation store
   * @param key
   *   a unique identifier to look for annotations
   * @param name
   *   the annotation name
   * @return
   *   either Some(annotation) when there is a matching annotation, or None
   */
