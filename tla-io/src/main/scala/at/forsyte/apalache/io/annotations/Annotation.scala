 ...
 *
 * <pre> \* @pure \* @type("(Int, Int) => Int") Plus(x, y) == x + y </pre>
 *
...
case class Annotation(name: String, args: AnnotationArg*) {
  ...
  /**
   * A pretty string representation. When the number of arguments is different from one, it behaves as #toString. When
   * there is one argument that is a string, it prints an annotation in the format: `@name: arg ;`
   *
   * @return
   */
