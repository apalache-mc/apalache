...
class TestQuint extends AnyFunSuite {
  ...
  test("Closed TNT record types are converted into closed TlaType1 records") {
    val record =
  ...
  test("Can convert types out of tictactoe.json") {
    ...
    val tntTypes = QuintOutput.read(tictactoeQuintJson).get.types
    // All type conversions go through
    tntTypes.map { case (id, t) => (id, t.typ, Quint.typeToTlaType(t.typ)) }
  }
}
