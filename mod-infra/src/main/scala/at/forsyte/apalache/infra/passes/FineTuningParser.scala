...
/**
 * A configuration class that mirrors the parameters that can be passed via `--tuning-options-file` and `--tuning-options`
 * to the commands `check` and `simulate`. This class serves as a data schema for the tuning options, which should
 * be properly parsed by [[https://pureconfig.github.io/docs/index.html PureConfig]].
 *
 * @see [[https://apalache-mc.org/docs/apalache/tuning.html Tuning parameters]].
...
object FineTuningParser {
  val fieldTypes: Map[String, ConfigFieldType] = immutable.Map(
    ...
    "z3.sat.lookahead.cube.cutoff" -> EnumFieldType("depth", "freevars", "psat", "adaptive_freevars", "adaptive_psat"),
