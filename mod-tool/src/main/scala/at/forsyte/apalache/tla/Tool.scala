...
object Tool extends App with LazyLogging {
  lazy val ISSUES_LINK: String =
    "[https://github.com/informalsystems/apalache/issues]"
  ...
  def run(args: Array[String]): Int = {
    Console.println(
      "# APALACHE version %s build %s".format(Version.version, Version.build)
    )
    Console.println("#")
    ...
    val command =
      Cli
        ...
        .version(
          "%s build %s".format(Version.version, Version.build),
          "version"
        )
        .withCommands(
          new ParseCmd,
      ...
      try {
        command match {
          case Some(parse: ParseCmd) =>
            ...
            submitStatisticsIfEnabled(
              Map("tool" -> "apalache", "mode" -> "parse", "workers" -> "1")
            )
          ...
          case Some(check: CheckCmd) =>
            ...
            submitStatisticsIfEnabled(
              Map("tool" -> "apalache", "mode" -> "check", "workers" -> "1")
            )
          ...
          case Some(typecheck: TypeCheckCmd) =>
            ...
            submitStatisticsIfEnabled(
              Map("tool" -> "apalache", "mode" -> "typecheck", "workers" -> "1")
            )
  ...
  private def printTimeDiff(startTime: LocalDateTime): Unit = {
    ...
    logger.info(
      "It took me %d days %2d hours %2d min %2d sec"
        .format(
          ChronoUnit.DAYS.between(startTime, endTime),
  ...
  private def runParse(injector: Injector, parse: ParseCmd, u: Unit): Unit = {
    ...
    if (result.isDefined) {
      ...
      logger.info(
        "Root module: %s with %d declarations"
          .format(tlaModule.name, tlaModule.declarations.length)
      )
    } else {
  ...
  private def runCheck(injector: Injector, check: CheckCmd, u: Unit): Unit = {
    ...
    executor.options.set("io.outdir", createOutputDir())
    var tuning =
      if (check.tuning != "") loadProperties(check.tuning)
      else Map[String, String]()
    tuning = overrideProperties(tuning, check.tuneHere)
    logger.info(
      "Tuning: " + tuning.toList.map { case (k, v) => s"$k=$v" }.mkString(":")
    )
    ...
    if (result.isDefined) {
      logger.info(
        "Checker reports no error up to computation length " + check.length
      )
    } else {
    ...
    try {
      ...
      case _: FileNotFoundException =>
        throw new PassOptionException(
          s"The properties file $filename not found"
        )

      case e: ConfigurationException =>
        throw new PassOptionException(
          s"Error in the properties file $filename: ${e.getMessage}"
        )
    }
  }

  private def overrideProperties(
      props: Map[String, String],
      propsAsString: String
  ): Map[String, String] = {
    def parseKeyValue(text: String): (String, String) = {
      val parts = text.split('=')
      if (parts.length != 2 || parts.head.trim == "" || parts(1) == "") {
        throw new PassOptionException(
          s"Expected key=value in --tune-here=$propsAsString"
        )
      } else {
  ...
  private def createOutputDir(): Path = {
    ...
    val nicetime = LocalDateTime
      .now()
      .format(DateTimeFormatter.ofPattern("HH.mm-dd.MM.uuuu-"))
  ...
  private def submitStatisticsIfEnabled(
