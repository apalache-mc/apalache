...
class RewriterBase extends FunSuite with BeforeAndAfterEach {
  ...
  override def beforeEach() {
    solverContext = new PreproSolverContext(
      new Z3SolverContext(SolverConfig.default.copy(debug = true))
    )
  ...
  protected def assertUnsatOrExplain(
  ...
  protected def assertTlaExAndRestore(
      rewriter: SymbStateRewriter,
      state: SymbState
  ): Unit = {
    rewriter.push()
  ...
  private def assertOrExplain(
    ...
    if (!outcome) {
      ...
      new SymbStateDecoder(solverContext, rewriter)
        .dumpArena(state, new PrintWriter(writer))
