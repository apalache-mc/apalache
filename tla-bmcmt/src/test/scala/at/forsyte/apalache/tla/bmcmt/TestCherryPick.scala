...
class TestCherryPick extends RewriterBase with TestingPredefs {
  ...
  private def assertEqWhenChosen(
    ...
    val ns =
      rewriter.rewriteUntilDone(state.setRex(tla.eql(state.ex, expected)))
    rewriter.push()
  ...
  test("""CHERRY-PICK {1, 2, 2} ~~> $B$k""") {
    ...
    val (oracleState, oracle) =
      new OracleFactory(rewriter).newConstOracle(state, 3)
    state = oracleState
  ...
  test("""CHERRY-PICK {<<1, 2>>, <<3, 4>>} ~~> $B$k""") {
    ...
    // introduce an oracle that tells us which element to pick
    val (oracleState, oracle) =
      new OracleFactory(rewriter).newConstOracle(state, 2)
    state = oracleState

    def mkTuple(i: Int, j: Int): ArenaCell = {
      state = rewriter.rewriteUntilDone(
        state.setRex(tla.tuple(tla.int(i), tla.int(j)))
      )
      state.asCell
    }
    ...
    state = new CherryPick(rewriter)
      .pickTuple(
        TupleT(Seq(IntT(), IntT())),
        state,
        oracle,
        tuples,
        state.arena.cellFalse()
      )
    assert(solverContext.sat())
  ...
  test("""CHERRY-PICK {<<1, <<2, 3>> >>, <<3, <<4, 5>> >>} ~~> $B$k""") {
    ...
    def mkTuple(i: Int, j: Int, k: Int): ArenaCell = {
      state = rewriter.rewriteUntilDone(
        state.setRex(tla.tuple(tla.int(i), tla.tuple(tla.int(j), tla.int(k))))
      )
      state.asCell
    }
    ...
    state = new CherryPick(rewriter).pickTuple(
      tupleT,
      state,
      oracle,
      tuples,
  ...
  test("""CHERRY-PICK-SEQ {<<1, 2>>, <<3, 4>>}""") {
    ...
    state = new CherryPick(rewriter).pickSequence(
      SeqT(IntT()),
      state,
      oracle,
      seqs,
  ...
  test("""CHERRY-PICK {[a |-> 1, b |-> 2], [a |-> 3, b |-> 4]} ~~> $B$k""") {
    ...
    def mkRecord(i: Int, j: Int): ArenaCell = {
      state = rewriter.rewriteUntilDone(
        state.setRex(
          tla.enumFun(tla.str("a"), tla.int(i), tla.str("b"), tla.int(j))
        )
      )
      state.asCell
    }
    ...
    state = new CherryPick(rewriter).pickRecord(
      records.head.cellType,
      state,
      oracle,
      records,
      state.arena.cellFalse()
  ...
  test("""CHERRY-PICK { {1, 2}, {3, 4} } ~~> $B$k""") {
    ...
    def mkSet(i: Int, j: Int): ArenaCell = {
      state = rewriter.rewriteUntilDone(
        state.setRex(tla.enumSet(tla.int(i), tla.int(j)))
      )
      state.asCell
    }

    val sets = Seq(mkSet(1, 2), mkSet(3, 4))
    state = new CherryPick(rewriter).pickSet(
      FinSetT(IntT()),
      state,
      oracle,
      sets,
  ...
  test("""CHERRY-PICK { {1, 2}, {} }""") {
    ...
    def mkSet(setEx: TlaEx): ArenaCell = {
      state = rewriter.rewriteUntilDone(
        state
          .setRex(tla.withType(setEx, AnnotationParser.toTla(FinSetT(IntT()))))
      )
      state.asCell
    }
  ...
  test("""CHERRY-PICK { {{1, 2}, {3, 4}}, {{5, 6}} }""") {
    ...
    val sets = Seq(
      rewriteEx(tla.enumSet(tla.enumSet(1, 2), tla.enumSet(3, 4))),
  ...
  test(
    """CHERRY-PICK { [x \in {1, 2} |-> 2 + x], [x \in {2, 3} |-> 2 * x] } ~~> $B$k"""
  ) {
    val rewriter = create()
    var state = new SymbState(tla.bool(true), arena, Binding())
    // introduce an oracle that tells us which element to pick
    ...
    def mkFun(dom: TlaEx, map: TlaEx): ArenaCell = {
      state = rewriter.rewriteUntilDone(
        state.setRex(tla.funDef(map, NameEx("x"), dom))
      )
      state.asCell
    }
    ...
    state = new CherryPick(rewriter).pickFun(
      funT,
      state,
      oracle,
      funs,
