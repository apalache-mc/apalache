...
class TestSymbStateRewriterSequence extends RewriterBase {
  ...
  test("""SE-SEQ-CTOR: <<>> <: Seq(Int)""") {
    ...
    val annotatedTuple =
      tla.withType(tuple, AnnotationParser.toTla(SeqT(IntT())))
  ...
  test("""SE-SEQ-APP: (<<3, 4, 5>> <: Seq(Int))[2]""") {
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.eql(tla.appFun(seq.toNameEx, tla.int(1)), tla.int(3))
      )
    )
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.eql(tla.appFun(seq.toNameEx, tla.int(2)), tla.int(4))
      )
    )
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.eql(tla.appFun(seq.toNameEx, tla.int(3)), tla.int(5))
      )
    )
  }
  ...
  test("""SE-SEQ-HEAD: Head(<<3, 4, 5>> <: Seq(Int))""") {
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(tla.eql(result.toNameEx, tla.int(3)))
    )
  }
  ...
  test("""SE-SEQ-TAIL: Tail(<<3, 4, 5>> <: Seq(Int))""") {
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.eql(tla.appFun(result.toNameEx, tla.int(1)), tla.int(4))
      )
    )
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.eql(tla.appFun(result.toNameEx, tla.int(2)), tla.int(5))
      )
    )
  }

  test(
    """regression: Tail(<<>> <: Seq(Int)) does not unsat and its length is zero"""
  ) {
    val emptyTuple = TlaFunOper.mkTuple()
    val annotatedTuple =
      tla.withType(emptyTuple, AnnotationParser.toTla(SeqT(IntT())))
    val seqTail = tla.tail(annotatedTuple)
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(tla.eql(tla.int(0), tla.len(nextState.ex)))
    )
  }
  ...
  test(
    """regression: SE-SEQ-SUBSEQ: SubSeq(S, 3, 1) does not unsat and has length 0"""
  ) {
    val tuple = TlaFunOper.mkTuple(3.to(6) map tla.int: _*)
    val annotatedTuple =
      tla.withType(tuple, AnnotationParser.toTla(SeqT(IntT())))
  ...
  test("""SE-SEQ-APPEND: Append(S, 10)""") {
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.eql(tla.appFun(result.toNameEx, tla.int(3)), tla.int(10))
      )
    )
    assertTlaExAndRestore(
      rewriter,
  ...
  test("""SE-SEQ-EQ: <<4, 5>> = SubSeq(<<3, 4, 5, 6>>, 2, 3)""") {
    ...
    val annot3456 =
      tla.withType(tuple3456, AnnotationParser.toTla(SeqT(IntT())))
  ...
  test("""SE-SEQ-DOMAIN: DOMAIN SubSeq(<<3, 4, 5, 6>>, 2, 3) = {2, 3}""") {
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.eql(tla.enumSet(tla.int(2), tla.int(3)), nextState.ex)
      )
    )
  }
  ...
  test("""SEQ-CONCAT: <<9, 10>> \o SubSeq(S, 2, 3)""") {
    ...
    val tuple9_10 =
      tla.withType(TlaFunOper.mkTuple(tla.int(9), tla.int(10)), seqT)
    ...
    val expected = tla.withType(
      TlaFunOper.mkTuple(tla.int(9), tla.int(10), tla.int(4), tla.int(5)),
      seqT
    )

    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(tla.eql(expected, nextState.ex))
    )
  }
