...
class TestSymbStateRewriterSet extends RewriterBase with TestingPredefs {
        ...
        nextState.ex match {
          case set @ NameEx(name) =>
            solverContext.assertGroundExpr(
              OperEx(TlaSetOper.in, arena.cellFalse().toNameEx, set)
            )
  ...
  test("""SE-SET-IN1: {3} \in {{1}, {3}, {5}} ~~> $B$k""") {
    val ex = tla.in(
      tla.enumSet(tla.int(3)),
      tla.enumSet(
        tla.enumSet(tla.int(1)),
        tla.enumSet(tla.int(3)),
        tla.enumSet(tla.int(5))
      )
    )
      ...
      case SymbStateRewriter.Continue(nextState) =>
        nextState.ex match {
          case predEx @ NameEx(name) =>
            ...
            // cell = \TRUE
            solverContext.assertGroundExpr(
              OperEx(TlaOper.eq, arena.cellTrue().toNameEx, cell.toNameEx)
            )
            ...
            // cell = \FALSE
            solverContext.assertGroundExpr(
              OperEx(TlaOper.eq, arena.cellFalse().toNameEx, cell.toNameEx)
            )
            // and membership holds true
            solverContext.assertGroundExpr(predEx)
            // contradiction
  ...
  test("""SE-SET-NOTIN1: c_i: ~(Bool \in {\TRUE, \TRUE}) ~~> c_pred""") {
    ...
    val ex =
      tla.not(
        tla.in(cell.toNameEx, tla.enumSet(tla.bool(true), tla.bool(true)))
      )
    val state = new SymbState(ex, arena, Binding())
    val rewriter = create()
    rewriter.rewriteUntilDone(state).ex match {
  ...
  ignore(
    """SE-SET-FILTER[1-2]: LET X = {1, 2} \cap {2} IN {} = {x \in X : [y \in X |-> TRUE][x]} ~~> $B$k"""
  ) {
    // regression
    ...
    val ex =
      OperEx(
        BmcOper.skolem,
        tla.letIn(
          tla.eql(tla.enumSet(), filteredSet),
  ...
  test("""SE-SET-FILTER: {Q \in Expand(SUBSET {1,2,3}) : ~(2 \in Q)}""") {
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.in(tla.enumSet(tla.int(1), tla.int(3)), nextState.ex)
      )
    )
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(
        tla.not(tla.in(tla.enumSet(tla.int(1), tla.int(2)), nextState.ex))
      )
    )
  }

  test(
    """SE-SET-FILTER[1-2]: \E SUBSET X {1} IN {} = {x \in X : [y \in X |-> TRUE][x]} ~~> $B$k"""
  ) {
    // regression
    ...
    val ex =
      OperEx(
        BmcOper.skolem,
        tla
          .exists("X", tla.powSet(baseSet), tla.eql(tla.enumSet(), filteredSet))
      )
  ...
  test(
    """SE-SET-FILTER[1-2]: \E X \in SUBSET {1, 2}: {} = {x \in X : [y \in {1} |-> TRUE][x]} ~~> $B$k"""
  ) {
    // regression
    val baseSet = tla.enumSet(1)
    val filter =
      tla.appFun(tla.funDef(tla.bool(true), "y", tla.enumSet(1)), "x")
    val filteredSet = tla.filter("x", "X", filter)
    val ex =
      OperEx(
        BmcOper.skolem,
  ...
  test(
    """SE-SET-MAP[1-2]: <<2, true>> \in {<<x, y>>: x \in {1,2,3}, y \in {FALSE, TRUE}} ~~> $B$k"""
  ) {
    ...
    val mappedSet =
      tla.map(mapping, tla.name("x"), set123, tla.name("y"), setBool)
  ...
  test(
    """MAP: \E S \in SUBSET { [a: "a", b: 1], [a: "a", b: 2] }:  "a" \in { r.a: r \in S }"""
  ) {
    ...
    val map = tla.map(
      tla.appFun(tla.name("r"), tla.str("a")),
      tla.name("r"),
      tla.name("S")
    )
    val mem = tla.in(tla.str("a"), map)
    val exists =
      OperEx(BmcOper.skolem, tla.exists(tla.name("S"), powerset, mem))
  ...
  test(
    ...
    val mem = tla.in(tla.str("a"), map)
    val forall = tla.forall(
      tla.name("x"),
      tla.name("S"),
      tla.eql(tla.appFun(tla.name("x"), tla.str("b")), tla.int(2))
    )
    val and = tla.and(mem, forall)
    val exists =
      OperEx(BmcOper.skolem, tla.exists(tla.name("S"), powerset, and))
  ...
  // rewritten by Keramelizer
  ignore(
    """SE-SUPSETEQ[1-3]: {1, 2, 3} \supseteq {1, 2, 3} ~~> $B$... (true)"""
  ) {
    val right = tla.enumSet(tla.int(1), tla.int(2), tla.int(3))
  ...
  test("""SE-UNION: UNION {{1, 2}, {2,3}} = {1, 2, 3}""") {
    ...
    val eq =
      OperEx(TlaOper.eq, tla.union(tla.enumSet(setOf12, setOf23)), setOf123)
    val rewriter = create()
