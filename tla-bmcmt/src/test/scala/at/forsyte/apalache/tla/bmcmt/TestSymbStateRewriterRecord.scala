...
class TestSymbStateRewriterRecord extends RewriterBase {
  test(
    """RecordDomainCache: ~(dom {"a", "b"} = dom {"a", "b", "c"}) ~~> $C$k"""
  ) {
    val rewriter = create()
    val (newArena1, set1) = rewriter.recordDomainCache
      .create(arena, (SortedSet("a", "b"), SortedSet[String]()))
    val (newArena2, set2) =
      rewriter.recordDomainCache
        .create(newArena1, (SortedSet("a", "b", "c"), SortedSet[String]()))
  ...
  test(
    """SE-REC-ACC[1-2]: ["a" |-> 1, "b" |-> FALSE, "c" |-> "d"]["c"] ~~> $C$k equals \"d\""""
  ) {
    val record = tla.enumFun(
      tla.str("a"),
      tla.int(1),
      ...
      case membershipEx @ NameEx(name) =>
        ...
        cell.cellType match {
          case BoolT() =>
            assertTlaExAndRestore(
              rewriter,
              nextState.setRex(tla.eql(cell.toNameEx, tla.bool(false)))
            )
  ...
  test(
    """SE-REC-CTOR[1-2] in a set: {["a" |-> 1, "b" |-> FALSE], ["a" |-> 2, "b" |-> TRUE]} ~~> $C$k"""
  ) {
    val record1 =
      tla.enumFun(tla.str("a"), tla.int(1), tla.str("b"), tla.bool(false))
    val record2 =
      tla.enumFun(tla.str("a"), tla.int(2), tla.str("b"), tla.bool(true))
  ...
  test(
    """SE-REC-CTOR[1-2] in a set: {["a" |-> 1, "b" |-> FALSE], ["a" |-> 2, "b" |-> TRUE, "c" |-> 3]} ~~> $C$k"""
  ) {
    val record1 =
      tla.enumFun(tla.str("a"), tla.int(1), tla.str("b"), tla.bool(false))
    val record2 = tla.enumFun(
      tla.str("a"),
      tla.int(2),
      tla.str("b"),
      tla.bool(true),
    ...
    val annotation = AnnotationParser.toTla(
      RecordT(SortedMap("a" -> IntT(), "b" -> BoolT(), "c" -> IntT()))
    )

    val state = new SymbState(
      tla.enumSet(tla.withType(record1, annotation), record2),
      arena,
      Binding()
    )
    val rewriter = create()
    ...
    nextState.ex match {
      case membershipEx @ NameEx(name) =>
        ...
        cell.cellType match {
          case FinSetT(rt @ RecordT(_)) =>
            assert(
              rt.fields == TreeMap("a" -> IntT(), "b" -> BoolT(), "c" -> IntT())
            )
  ...
  test(
    """SE-REC-CTOR[1-2] type error: {["a" |-> FALSE, "b" |-> 1], ["a" |-> 2, "b" |-> TRUE]} ~~> $C$k"""
  ) {
    val record1 =
      tla.enumFun(tla.str("a"), tla.bool(false), tla.str("b"), tla.int(1))
    val record2 =
      tla.enumFun(tla.str("a"), tla.int(2), tla.str("b"), tla.bool(true))
  ...
  test(
    ...
    val record2 =
      tla.enumFun(tla.str("a"), tla.int(2), tla.str("c"), tla.int(3))
    // Records in a set can have different sets of keys. This requires a type annotation.
    val annotation =
      AnnotationParser.toTla(RecordT(SortedMap("a" -> IntT(), "c" -> IntT())))
    ...
    val mapEx = tla.map(
      tla.appFun(tla.name("r"), tla.str("c")),
      tla.name("r"),
      filteredEx
    )
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(tla.eql(nextState.ex, tla.enumSet(tla.int(3))))
    )
    rewriter.pop()
    rewriter.push()
    ...
    assert(
      cell.cellType == FinSetT(RecordT(SortedMap("a" -> IntT(), "c" -> IntT())))
    )
  }

  test(
    """SE-REC-EQ: [a |-> 1, b |-> FALSE, c |-> "d"] = [c |-> "d", b |-> FALSE, a |-> 1] ~~> TRUE"""
  ) {
    val record1 = tla.enumFun(
      tla.str("a"),
      tla.int(1),
      tla.str("b"),
      tla.bool(false),
  ...
  ignore(
    ...
    val eq = tla.eql(
      prod,
      tla.enumSet(rec(1, false), rec(1, true), rec(2, false), rec(2, true))
    )
  ...
  ignore(
    ...
    val expectedRecordT =
      FinSetT(RecordT(SortedMap("n" -> IntT(), "b" -> BoolT())))
  ...
  ignore("""SE-REC-SET: x' \in {[n |-> Int, b |-> BOOLEAN ]}""".stripMargin) {
    ...
    val inInt =
      tla.in(
        tla.appFun(tla.prime(tla.name("x")), tla.str("n")),
    ...
    val inBool =
      tla.in(
        tla.appFun(tla.prime(tla.name("x")), tla.str("b")),
  ...
  test(
    """SE-REC-DOM: DOMAIN [a |-> 1, b |-> FALSE, c |-> "d"] = {"a", "b", "c"}"""
  ) {
    val record = tla.enumFun(
      tla.str("a"),
      tla.int(1),
  ...
  test(
    """SE-REC-DOM: DOMAIN ([a |-> 1] <: [a |-> 1, b |-> FALSE, c |-> "d"]) = {"a", "b", "c"}"""
  ) {
    val record = tla.enumFun(
      tla.str("a"),
      tla.int(1),
    ...
    val richerType = AnnotationParser.toTla(
      RecordT(SortedMap("a" -> IntT(), "b" -> BoolT(), "c" -> ConstT()))
    )
    val annotated =
  ...
  test("""SE-REC-EXCEPT:[ ["a" |-> 1, "b" |-> FALSE] EXCEPT !["a"] = 3 ]""") {
    ...
    val nextState = rewriter.rewriteUntilDone(state)
    val expectedRec =
      tla.enumFun(tla.str("a"), tla.int(3), tla.str("b"), tla.bool(false))
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(tla.eql(expectedRec, nextState.ex))
    )
  }
