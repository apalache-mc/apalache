...
class TestSymbStateRewriterAssignment extends RewriterBase with TestingPredefs {
  ...
  test(
    """SE-IN-ASSIGN1(int): \E t \in {1, 2}: x' \in {t} ~~> TRUE and [x -> $C$k]"""
  ) {
    val set = set12
    val assign = OperEx(
      BmcOper.skolem,
  ...
  test("""SE-IN-ASSIGN1(int): x' \in {} ~~> FALSE""") {
    val assign = OperEx(
      BmcOper.skolem,
      tla.exists(boundName, tla.enumSet(), tla.assign(x_prime, boundName))
    )
  ...
  test(
    ...
    val assign = OperEx(
      ...
      tla.exists(
        boundName,
        empty(tla.enumSet(tla.int(1))),
        tla.assign(x_prime, boundName)
      )
    )
  ...
  test(
    """SE-IN-ASSIGN1(set): \E t \in {{1, 2}, {2, 3}}: x' \in {t} ~~> TRUE and [x -> $C$k]"""
  ) {
  ...
  test(
    """SE-IN-ASSIGN1(set): \E t \in {{1, 2}, {1+1, 2, 3}} \ {{2, 3}}: x' \in {t} ~~> TRUE and [x -> $C$k]"""
  ) {
    ...
    val twoSets =
      tla.enumSet(tla.enumSet(1, 2), tla.enumSet(tla.plus(1, 1), 2, 3))
  ...
  test(
    """SE-IN-ASSIGN1(set): \E t \in SUBSET {1, 2}: x' \in {t} ~~> TRUE and [x -> $C$k]"""
  ) {
    ...
    rewriter.push()
    val eq13 =
      tla.eql(boundCell, tla.enumSet(tla.int(1), tla.int(2), tla.int(3)))
    val eqState13 = rewriter.rewriteUntilDone(nextState.setRex(eq13))
    solverContext.assertGroundExpr(eqState13.ex)
    assertUnsatOrExplain(rewriter, eqState13) // should not be possible
  }
  ...
  test(
    """SE-IN-ASSIGN1(fun): \E t \in {[x \in BOOLEAN |-> 0], [x2 \in BOOLEAN |-> 1]}: x' \in {t} ~~> TRUE"""
  ) {
  ...
  test(
    """SE-IN-ASSIGN1(funset): \E t \in [BOOLEAN -> {0, 1}]: x' \in {t} ~~> TRUE"""
  ) {
  ...
  test(
    """SE-IN-ASSIGN1(funset): \E t \in [{} -> {0, 1}]: x' \in {t} ~~> FALSE"""
  ) {
    // regression
  ...
  test(
    """SE-IN-ASSIGN1(funset): \E t \in [0..(5 - 1) -> BOOLEAN]: x' \in {t} ~~> TRUE"""
  ) {
  ...
  test("""ASSIGN[funset with Nat]: \E t \in [0..4 -> Nat]: x' <- t""") {
    ...
    assertTlaExAndRestore(
      rewriter,
      nextState.setRex(tla.ge(tla.appFun(x, tla.int(1)), 0))
    )
  }
  ...
  test(
    """SE-IN-ASSIGN1(tuple): \E t \in {<<1, FALSE, {1, 3}>>, <<2, TRUE, {4}>>}: x' \in {t}"""
  ) {
    ...
    nextState.ex match {
      case NameEx(_) =>
        ...
        val membershipTest =
          tla.and(
            tla.in(tla.appFun(x_prime, tla.int(1)), set12),
            tla.in(tla.appFun(x_prime, tla.int(2)), boolset),
  ...
  test(
    ...
    val annotation = AnnotationParser.toTla(
      RecordT(SortedMap("a" -> IntT(), "b" -> BoolT(), "c" -> FinSetT(IntT())))
    )
      ...
      case NameEx(_) =>
        ...
        val (newArena, expectedDom) =
          rewriter.recordDomainCache.getOrCreate(
            assumeState2.arena,
            (SortedSet("a", "b"), SortedSet("c"))
          )
        val domEq = tla.eql(expectedDom, tla.dom(x_prime))
        assertTlaExAndRestore(
          rewriter,
          assumeState2.setArena(newArena).setRex(domEq)
        )
