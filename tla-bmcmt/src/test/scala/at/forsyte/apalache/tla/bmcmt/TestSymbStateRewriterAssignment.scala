...
class TestSymbStateRewriterAssignment extends RewriterBase {
  ...
  test("""\E t \in {1, 2}: x' \in {t} ~~> TRUE and [x -> $C$k]""") { rewriter: SymbStateRewriter =>
  ...
  test("""assign in conjunction""") { rewriter: SymbStateRewriter =>
    val and1 =
      and(
  ...
  test("""\E t \in {}: x' = t ~~> FALSE""") { rewriter: SymbStateRewriter =>
    val asgn =
  ...
  test("""\E t \in \in {t_2 \in {1}: FALSE}: x' \in {t} ~~> FALSE""") { rewriter: SymbStateRewriter =>
    // a regression test
  ...
  test("""x' \in {1} /\ x' = 1 ~~> TRUE and [x -> $C$k]""") { rewriter: SymbStateRewriter =>
  ...
  test("""\E t \in {{1, 2}, {2, 3}}: x' \in {t} ~~> TRUE and [x -> $C$k]""") { rewriter: SymbStateRewriter =>
  ...
  test("""\E t \in SUBSET {1, 2}: x' \in {t} ~~> TRUE and [x -> $C$k]""") { rewriter: SymbStateRewriter =>
  ...
  test("""\E t \in [BOOLEAN -> {0, 1}]: x' \in {t} ~~> TRUE""") { rewriter: SymbStateRewriter =>
  ...
  test("""\E t \in [{} -> {0, 1}]: x' \in {t} ~~> FALSE""") { rewriter: SymbStateRewriter =>
    // regression
  ...
  test("""\E t \in [0..(5 - 1) -> BOOLEAN]: x' \in {t} ~~> TRUE""") { rewriter: SymbStateRewriter =>
  ...
  test("""\E t \in [0..4 -> Nat]: x' <- t""") { rewriter: SymbStateRewriter =>
  ...
  test("""\E t \in [0..4 -> Int]: x' <- t""") { rewriter: SymbStateRewriter =>
    val domain = dotdot(int(0), int(4)).typed(types, "I")
  ...
  test("""\E t \in {<<1, FALSE, {1, 3}>>, <<2, TRUE, {4}>>}: x' = t""") { rewriter: SymbStateRewriter =>
