...
trait TestRecordingSolverContext extends AnyFunSuite {
  ...
  test("operations proxied") {
    val solver = RecordingSolverContext.create(None, solverConfig, OutputWorkspaceMock)
  ...
  test("write and read") {
    ...
    assert(!solver.sat())
    // restore the context
    val restoredSolver = RecordingSolverContext.create(Some(log), solverConfig, OutputWorkspaceMock)
