...
/**
  ...
  * @author Igor Konnov
  */
class QuantRule(rewriter: SymbStateRewriter)
  ...
  override def apply(state: SymbState): SymbState = {
    state.ex match {
      case OperEx(
          BmcOper.`skolem`,
          OperEx(TlaBoolOper.exists, NameEx(boundVar), boundingSetEx, predEx)
          ) =>
        ...
        boundingSetEx match {
          ...
          case _ =>
            val setState =
              rewriter.rewriteUntilDone(state.setRex(boundingSetEx))
            ...
            set.cellType match {
              ...
              case tp =>
                throw new UnsupportedOperationException(
                  "Quantification over %s is not supported yet".format(tp)
                )
            }
        }
  ...
  private def isAssignmentInside(ex: TlaEx): Boolean = ex match {
    case OperEx(BmcOper.assign, OperEx(TlaActionOper.prime, NameEx(_)), _) =>
      true
  ...
  private def expandExistsOrForall(
    ...
    rewriter.solverContext.log(
      "; quantification over a finite set => expanding"
    )
    ...
    set.cellType match {
      ...
      case tp =>
        val msg = "Trying to expand a complex set in \\E %s \\in ... ".format(
          boundVar
        ) +
          "Try to move \\E out of a negated expression. Expression: "
        logger.warn(msg)
        throw new UnsupportedOperationException(
          "Expansion of %s is not supported yet".format(tp)
        )
    }
    ...
    if (setCells.isEmpty) {
      ...
      def mkPair(elemCell: ArenaCell): (Binding, TlaEx) = {
        val newBinding = Binding(
          setState.binding.toMap + (boundVar -> elemCell)
        )
        (newBinding, predEx)
      }
  ...
  private def skolemExistsInNatOrInt(
    ...
    rewriter.solverContext.log(
      "; free existential rule over an infinite set " + boundingSetEx
    )
    ...
    val extendedBinding = Binding(
      nextState.binding.toMap + (boundVar -> witness)
    )
  ...
  private def skolemExistsInRange(
    ...
    rewriter.solverContext.log(
      s"; skolemizable existential $boundVar over $leftBound..$rightBound "
    )
    var nextState = state.setArena(state.arena.appendCell(IntT()))
    val witness = nextState.arena.topCell
    // assert that the witness is in the range leftBound..rightBound
    nextState =
      rewriter.rewriteUntilDone(nextState.setRex(tla.ge(witness, leftBound)))
    rewriter.solverContext.assertGroundExpr(nextState.ex)
    nextState =
      rewriter.rewriteUntilDone(nextState.setRex(tla.le(witness, rightBound)))
    rewriter.solverContext.assertGroundExpr(nextState.ex)
    // enforce that the witness satisfies the predicate
    val extendedBinding = Binding(
      nextState.binding.toMap + (boundVar -> witness)
  ...
  private def skolemExistsInSet(
    ...
    if (setCells.isEmpty) {
      ...
      skolemExistsInStaticallyNonEmptySet(
        setState,
        boundVar,
        predEx,
        set,
  ...
  private def skolemExistsInStaticallyNonEmptySet(
    ...
    val (oracleState, oracle) =
      pickRule.oracleFactory.newDefaultOracle(setState, setCells.size + 1)
    var nextState = oracleState
    OracleHelper.assertOraclePicksSetMembers(
      rewriter,
      nextState,
      oracle,
      set,
    ...
    nextState = pickRule.pickByOracle(
      nextState,
      oracle,
      setCells,
      nextState.arena.cellTrue()
    )
    val pickedCell = nextState.asCell
    // enforce that the witness satisfies the predicate
    val extendedBinding = Binding(
      nextState.binding.toMap + (boundVar -> pickedCell)
    )
    // predState.ex contains the predicate applied to the witness
    nextState = rewriter.rewriteUntilDone(
      nextState
  ...
  private def skolemExistsByPick(
    ...
    // enforce that the witness satisfies the predicate
    val extendedBinding = Binding(
      pickState.binding.toMap + (boundVar -> pickedCell)
    )
    // predState.ex contains the predicate applied to the witness
