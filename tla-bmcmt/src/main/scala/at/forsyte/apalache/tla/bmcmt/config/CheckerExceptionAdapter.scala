...
import at.forsyte.apalache.tla.pp.{
  ConfigurationError,
  IrrecoverablePreprocessingError,
  NotInKeraError,
  TlaInputError
}
...
/**
  ...
  */
@Singleton
class CheckerExceptionAdapter @Inject()(
    sourceStore: SourceStore,
    changeListener: ChangeListener
) extends ExceptionAdapter
    with LazyLogging {
  private lazy val ISSUES_LINK: String =
  ...
  override def toMessage: PartialFunction[Exception, ErrorMessage] = {
    ...
    case err: ConfigurationError =>
      NormalErrorMessage(
        "Configuration error (see the manual): " + err.getMessage
      )
    ...
    case err: TypeInferenceException =>
      val msg = "%s\n%s".format(
        err.getMessage,
        err.errors.map(ofTypeInferenceError).mkString("\n")
      )
    ...
    case err: LanguagePredError =>
      ...
      err.failedIds.foreach { idAndMsg =>
        logger.error(
          "%s: unexpected expression: %s"
            .format(findLoc(idAndMsg._1), idAndMsg._2)
        )
      }
      NormalErrorMessage(
    ...
    case err: NotInKeraError =>
      NormalErrorMessage(
        "%s: Input error (see the manual): %s"
          .format(findLoc(err.causeExpr.ID), err.getMessage)
      )

    // tool failures
    case err: IrrecoverablePreprocessingError =>
      val msg =
        s"Irrecoverable preprocessing error: ${err.getMessage}. Report an issue at $ISSUES_LINK"
      FailureMessage(msg)

    case err: NoRuleException =>
      val msg =
        "%s: no rule to rewrite a TLA+ expression: %s".format(
          findLoc(err.causeExpr.ID),
          err.getMessage
        )
      FailureMessage(msg)

    case err: RewriterException =>
      val msg = "%s: rewriter error: %s".format(
        findLoc(err.causeExpr.ID),
        err.getMessage
      )
      FailureMessage(msg)
    ...
    case err: SmtEncodingException =>
      val msg = "%s: error when rewriting to SMT: %s".format(
        findLoc(err.causeExpr.ID),
        err.getMessage
      )
      FailureMessage(msg)
    ...
    case err: TypeException =>
      FailureMessage(
        "%s: type error: %s".format(findLoc(err.causeExpr.ID), err.getMessage)
      )

    case err: InvalidTlaExException =>
      val msg = "%s: unexpected TLA+ expression: %s".format(
        findLoc(err.causeExpr.ID),
        err.getMessage
      )
      FailureMessage(msg)
    ...
    case err: InternalCheckerError =>
      val msg = "%s: internal error: %s".format(
        findLoc(err.causeExpr.ID),
        err.getMessage
      )
      FailureMessage(msg)
    ...
    case err: CheckerException =>
      val msg = "%s: checker error: %s".format(
        findLoc(err.causeExpr.ID),
        err.getMessage
      )
      FailureMessage(msg)
