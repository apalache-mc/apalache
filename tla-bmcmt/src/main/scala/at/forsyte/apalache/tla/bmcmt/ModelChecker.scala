...
import at.forsyte.apalache.tla.bmcmt.smt.{
  SolverConfig,
  SolverContext,
  Z3SolverContext
}
...
class ModelChecker(
  ...
  private val rewriter: SymbStateRewriterImpl =
    new SymbStateRewriterImpl(
      ...
      if (profile) {
        Some(
          new MetricProfilerListener(
            sourceStore,
            changeListener,
            new File("profile.csv")
          )
        )
      } else {
        None
  ...
  private val invariantSplitByTransition: Boolean =
    tuningOptions
      .getOrElse("search.invariant.split", "true")
      .toLowerCase == "true"

  private val learnTransFromUnsat: Boolean =
    tuningOptions
      .getOrElse("search.transition.learnFromUnsat", "")
      .toLowerCase == "true"

  private val learnInvFromUnsat: Boolean =
    tuningOptions
      .getOrElse("search.invariant.learnFromUnsat", "")
      .toLowerCase == "true"
  ...
  /**
    ...
    */
  private val constants = Set(
    checkerInput.rootModule.constDeclarations.map(_.name): _*
  )
  ...
  private def initializeConstants(state: SymbState): SymbState = {
    val newState =
      if (checkerInput.constInitPrimed.isEmpty) {
        ...
        val nextState = rewriter.rewriteUntilDone(
          state.setRex(checkerInput.constInitPrimed.get)
        )
    ...
    if (uninitialized.nonEmpty) {
      logger.error(
        "The following CONSTANTS are not initialized: " + uninitialized
          .mkString(", ")
      )
    ...
    searchStrategy.getCommand match {
      ...
      case FinishOnDeadlock() =>
        ...
        if (solverContext.sat()) {
          ...
          logger.error(
            s"Check an execution leading to a deadlock state in any of ${filenames.mkString(", ")}"
          )
        } else {
      ...
      case NextStep(
        ...
        val transitions =
          if (stepNo == 0) filter(checkerInput.initTransitions)
          else filter(checkerInput.nextTransitions)

        // make the step
        val transWithEnabled =
          findEnabledOrBugs(stepNo, state, transitions.toList)
  ...
  private def findEnabledOrBugs(
      stepNo: Int,
      startingState: SymbState,
      transitionsAndNos: List[(TlaEx, Int)]
  ): List[((TlaEx, Int), Boolean)] = {
    // find all the feasible transitions and check the invariant for each transition
    logger.info(
      "Step %d, level %d: checking if %d transition(s) are enabled and violate the invariant"
      ...
      ts match {
        ...
        case tranWithNo :: tail =>
          ...
          if (!stepMatchesFilter(stepNo, transitionNo)) {
            ...
            val (nextState, isEnabled) = applyTransition(
              stepNo,
              erased,
              transition,
              transitionNo,
            ...
            rewriter
              .pop() // forget all the constraints that were generated by the transition, LEVEL + 0
  ...
  private def applyEnabled(
      stepNo: Int,
      startingState: SymbState,
      transWithEnabled: List[((TlaEx, Int), Boolean)]
  ): Option[SymbState] = {
    // second, apply the enabled transitions and collect their effects
    logger.info(
      "Step %d, level %d: collecting %d enabled transition(s)"
      ...
      ts match {
        ...
        case (tranWithNo, isEnabled) :: tail =>
          if (!isEnabled && !learnTransFromUnsat) {
            ...
            var (nextState, _) = applyTransition(
              stepNo,
              erased,
              transition,
              transitionNo,
            ...
            if (isEnabled) {
              ...
              solverContext.assertGroundExpr(
                simplifier.simplifyShallow(tla.not(nextState.ex))
              )
    ...
    if (nextStates.isEmpty) {
      ...
      solverContext.assertGroundExpr(
        oracle.caseAssertions(oracleState, nextStates.map(_.ex))
      )
      ...
      def getAssignedVars(st: SymbState) =
        forgetNonPrimed(st.binding, Set()).toMap.keySet
      ...
      if (nextStates.exists(getAssignedVars(_) != primedVars)) {
        ...
        val diff =
          otherSet.union(primedVars).diff(otherSet.intersect(primedVars))
        val msg =
      ...
      def pickVar(x: String): ArenaCell = {
        ...
        finalState = picker.pickByOracle(
          finalState,
          oracle,
          toPickFrom,
          finalState.arena.cellFalse().toNameEx
        ) // no else case
        finalState.asCell
      }
      ...
      val constBinding =
        oracleState.binding.toMap.filter(p => constants.contains(p._1))
      finalState =
        finalState.setBinding(Binding(finalVarBinding.toMap ++ constBinding))
      ...
      if (!invariantSplitByTransition) {
        checkAllInvariants(stepNo, 0, finalState)
      }
      ...
      finalState =
        finalState.setBinding(shiftBinding(finalState.binding, constants))
  ...
  private def applyTransition(
    ...
    if (checkForErrors && debug) {
      ...
      logger.debug(
        "Finished rewriting. Checking satisfiability of the pushed constraints."
      )
      ...
      solverContext.satOrTimeout(transitionTimeout) match {
        ...
        case r: Option[Boolean] => // unsat or timeout
          ...
          if (r.isDefined) {
            ...
            logger.debug(
              s"Timed out when checking feasibility of transition #$transitionNo. Assuming it is infeasible."
            )
          }
          rewriter.pop() // LEVEL + 0
          solverContext.log(
  ...
  private def assumeInvariant(stepNo: Int, state: SymbState): SymbState = {
    val matchesInvFilter = invFilter == "" || stepNo.toString.matches(
      "^" + invFilter + "$"
    )
  ...
  private def checkAllInvariants(
      stepNo: Int,
      transitionNo: Int,
      nextState: SymbState
  ): Unit = {
    ...
    val prevMatchesInvFilter =
      stepNo > 0 && (invFilter == "" || (stepNo - 1).toString
        .matches("^" + invFilter + "$"))
    ...
    for ((notInv, invNo) <- invNegs.zipWithIndex) {
      ...
      shiftTypes(constants)
      val shiftedState =
        nextState.setBinding(shiftBinding(nextState.binding, constants))
      rewriter.exprCache
        .disposeActionLevel() // renaming x' to x makes the cache inconsistent, so clean it
  ...
  private def checkOneInvariant(
      stepNo: Int,
      transitionNo: Int,
      nextState: SymbState,
      changedPrimed: Set[String],
      notInv: TlaEx
  ): Unit = {
    ...
    if (stepNo != 0 && used.nonEmpty && used.intersect(changedPrimed).isEmpty) {
      ...
      logger.debug(
        s"The invariant is referring only to the UNCHANGED variables. Skipped."
      )
    } else {
      rewriter.push()
      val notInvState = rewriter.rewriteUntilDone(
        nextState
          .setRex(notInv)
      )
      ...
      solverContext.satOrTimeout(invariantTimeout) match {
        case Some(true) =>
          ...
          logger.error(
            s"Invariant is violated at depth $stepNo. Check the counterexample in any of ${filenames
              .mkString(", ")}"
          )
          if (debug) {
            logger.warn(
              s"Dumping the arena into smt.log. This may take some time..."
            )
            // dump everything in the log
            val writer = new StringWriter()
            new SymbStateDecoder(solverContext, rewriter)
              .dumpArena(notInvState, new PrintWriter(writer))
  ...
  private def shiftTypes(constants: Set[String]): Unit = {
    ...
    val nextTypes =
      types
        .filter(p => p._1.endsWith("'") || constants.contains(p._1))
  ...
  private def shiftBinding(
      binding: Binding,
      constants: Set[String]
  ): Binding = {
    Binding(
      forgetNonPrimed(binding, constants).toMap
        .map(p => (p._1.stripSuffix("'"), p._2))
    )
  }
  ...
  private def dumpEnabledMap(): Unit = {
    ...
    val hrule =
      "----%s".format((0 until transitionsCnt map (_ => "-")) mkString "")
    writer.println(hrule)
    writer.println(
      "    %s".format(
        0 until transitionsCnt map (i => ((i / 100) % 10).toString) mkString ""
      )
    )
    writer.println(
      "    %s".format(
        0 until transitionsCnt map (i => ((i / 10) % 10).toString) mkString ""
      )
    )
    writer.println(
      "    %s"
        .format(0 until transitionsCnt map (i => (i % 10).toString) mkString "")
    )
    writer.println(hrule)
    for ((enabled, stepNo) <- enabledList.zipWithIndex) {
      val set = Set(enabled: _*)
      val line = 0 until transitionsCnt map (
          i => if (set.contains(i)) "*" else " "
      ) mkString ""
