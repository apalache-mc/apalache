...
class SeqOpsRule(rewriter: SymbStateRewriter) extends RewritingRule {
  ...
  private def translateTail(state: SymbState, seq: TlaEx) = {
    ...
    val updatedStart =
      tla.ite(tla.lt(start, end), tla.plus(tla.int(1), start), start)
    // increment start
    ...
    nextState = nextState.updateArena(
      _.appendHasNoSmt(newSeqCell, newStart +: cells.tail: _*)
    )
  ...
  private def translateSubSeq(
    ...
    val expectedStart = rewriteToCell(
      tla.plus(start, tla.minus(newStartEx, tla.int(1)))
    )
    ...
    val newStart = rewriteToCell(
      tla.ite(seqInvariant, expectedStart, tla.int(0))
    )
    ...
    val newSeqCell = nextState.arena.topCell
    nextState = nextState.updateArena(
      _.appendHasNoSmt(newSeqCell, newStart :: newEnd +: cells.tail.tail: _*)
    )
    nextState.setRex(newSeqCell)
  }
  ...
  private def translateAppend(state: SymbState, seq: TlaEx, newElem: TlaEx) = {
    ...
    nextState =
      rewriter.rewriteUntilDone(nextState.setRex(tla.plus(tla.int(1), end)))
    ...
    val newSeqCell = nextState.arena.topCell
    nextState = nextState.updateArena(
      _.appendHasNoSmt(newSeqCell, start :: newEnd +: newCells: _*)
    )
    nextState.setRex(newSeqCell)
  }
  ...
  private def translateConcat(
    ...
    def rewriteSeq(
        seqEx: TlaEx
    ): (ArenaCell, ArenaCell, ArenaCell, Seq[ArenaCell], CellT) = {
    ...
    nextState =
      rewriter.rewriteUntilDone(nextState.setRex(tla.plus(len1, len2)))
    ...
    nextState = rewriter.rewriteUntilDone(
      nextState.setRex(tla.minus(tla.plus(tla.int(elems1.size), start2), len1))
    )
    ...
    def pickElem(i: Int): ArenaCell = {
      // create the oracle
      val (oracleState, oracle) =
        picker.oracleFactory.newIntOracle(nextState, ntotal + 1)
      nextState = oracleState
      ...
      val whenInRange1 =
        tla.or(
          tla.not(inRange1),
          tla.eql(oracle.intCell, tla.plus(tla.int(i), start1))
        )
      val whenInRange2 =
        tla.or(
          tla.not(inRange2),
    ...
    nextState = nextState.updateArena(
      _.appendHasNoSmt(seq3, start3 +: end3 +: newCells: _*)
    )
