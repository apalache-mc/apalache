...
//    val cinitStrs = cinits.map(TermToVMTWriter.mkSMT2String)
    ...
    outputWorkspace.withWriterInRunDir(TlaExToVMTWriter.outFileName) { writer =>
      ...
      writeLine(";Sorts")
      sortDecls.foreach(writeLine)
      writer.newLine()
      writeLine(";Constants")
      ulitDecls.foreach(writeLine)
      writer.newLine()
      disticntAsserts.foreach(writeLine)
      writer.newLine()
      writeLine(";Variables")
      smtVarDecls.foreach(writeLine)
      writer.newLine()
      writeLine(";Variable bindings")
      nextStrs.foreach(writeLine)
      writer.newLine()
//      writeLine(";TLA constant initialization")
//      cinitStrs.foreach(writeLine)
//      writer.newLine()
      writeLine(";Initial states")
      initStrs.foreach(writeLine)
      writer.newLine()
      writeLine(";Transitions")
      transStrs.foreach(writeLine)
      writer.newLine()
      writeLine(";Invariants")
      invStrs.foreach(writeLine)
    }
  }
