...
class RecCtorRule(rewriter: SymbStateRewriter) extends RewritingRule {
  ...
  override def apply(state: SymbState): SymbState = {
    state.ex match {
      case OperEx(TlaFunOper.enum, elems @ _*) =>
        ...
        val typeArgs = elems.zipWithIndex.map(
          p => if (p._2 % 2 == 0) ConstT() else valueCells(p._2 / 2).cellType
        )
        val recordT = rewriter.typeFinder
          .compute(state.ex, typeArgs: _*)
          .asInstanceOf[RecordT]
        ...
        def addExtra(map: Map[String, ArenaCell], key: String) = {
          ...
          val (newArena, keyCell) =
            rewriter.strValueCache.getOrCreate(arena, key)
          arena = newArena
          map + (key -> keyCell)
        }
        // map these extra keys to the respective cells
        val extraKeyMap =
          extraKeys.foldLeft(Map.empty[String, ArenaCell])(addExtra)
        ...
        def addField(key: String, tp: CellT): Unit = {
          ...
          nextState =
            nextState.updateArena(_.appendHasNoSmt(recordCell, valueCell))
        }
        ...
        val (newArena, domain) =
          rewriter.recordDomainCache
            .getOrCreate(nextState.arena, (SortedSet(ctorKeys: _*), extraKeys))
        nextState = nextState.setArena(newArena.setDom(recordCell, domain))
        // importantly, the record keys that are outside of ctorKeys should not belong to the domain!
        if (extraKeyMap.nonEmpty) {
          val extraOutsideOfDomain =
            extraKeyMap.values.map(f => tla.notin(f.toNameEx, domain.toNameEx))
          rewriter.solverContext.assertGroundExpr(
            tla.and(extraOutsideOfDomain.toSeq: _*)
          )
        }
  ...
  private def keysToStr(ex: TlaEx, keys: List[TlaEx]): List[String] = {
    def eachKey(k: TlaEx) = k match {
      ...
      case _ =>
        throw new RewriterException(
          "Expected a string as a record key, found: " + k,
          ex
        )
    }
