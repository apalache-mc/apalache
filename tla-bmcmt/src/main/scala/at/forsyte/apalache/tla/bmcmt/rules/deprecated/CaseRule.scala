...
class CaseRule(rewriter: SymbStateRewriter) extends RewritingRule {
  ...
  override def apply(state: SymbState): SymbState = {
    def decorateWithIf(
        elseEx: TlaEx,
        guardAction: Tuple2[TlaEx, TlaEx]
    ): OperEx = {
    ...
    state.ex match {
      ...
      case OperEx(TlaControlOper.caseNoOther, args @ _*) =>
        ...
        val iteWaterfall =
          revGuardsAndActions.foldLeft(assertState.ex)(decorateWithIf)
  ...
  private def mkGuardsAndActions(
      args: Seq[TlaEx]
  ): Seq[Tuple2[TlaEx, TlaEx]] = {
  ...
  private def getCellType(state: SymbState): CellT = {
    state.ex match {
      ...
      case _ =>
        throw new RewriterException(
          "The rewriting result is not a cell but %s" + state.ex,
          state.ex
        )
    }
  }
