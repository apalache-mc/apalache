          ...
          if (potentialElems.isEmpty) {
            ...
            val pred = state.solverCtx.introBoolConst()
              if (newArena.isLinkedViaHas(setCell, candCell)) {
                ...
                val eqState = rewriter.lazyEquality.cacheEqualities(setState, potentialElems.map(e => (e, candCell)))

                def inAndEq(elem: ArenaCell) = {
                  tla.and(tla.in(elem, setCell), tla.eql(elem, candCell)) // equalities are generated by lazy equality
                }
