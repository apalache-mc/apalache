...
class IntArithRule(rewriter: SymbStateRewriter) extends RewritingRule {
  ...
  override def isApplicable(symbState: SymbState): Boolean = {
    symbState.ex match {
      case OperEx(TlaArithOper.plus, _, _) | OperEx(TlaArithOper.minus, _, _) |
          OperEx(TlaArithOper.mult, _, _) | OperEx(TlaArithOper.div, _, _) |
          OperEx(TlaArithOper.mod, _, _) | OperEx(TlaArithOper.exp, _, _) |
  ...
  private def rewriteGeneral(state: SymbState, ex: TlaEx) = ex match {
    ...
    case OperEx(TlaArithOper.uminus, subex) =>
      ...
      rewriter.solverContext.assertGroundExpr(
        tla.eql(newCell.toNameEx, tla.uminus(subState.ex))
      )
    ...
    case OperEx(oper: TlaArithOper, left, right) =>
      ...
      val cons =
        tla.eql(newCell.toNameEx, OperEx(oper, leftState.ex, rightState.ex))
      rewriter.solverContext.assertGroundExpr(cons)
      rightState.setArena(newArena).setRex(newCell.toNameEx)

    case _ =>
      throw new RewriterException(
        "It should not happen. Report a bug",
        state.ex
      )
  }
}
