 ...
 * uninterpreted constants. Now we are using just propositional variables. This may change again in the future.
 *
 * @author
 *   Igor Konnov
 */
object OracleHelper {
   ...
   *
   * <ul> <li>oracle = i > in(e_i, S) for 0 <= i < N, and</li> <li>oracle = N => \A i \in 0..(N-1) ~in(e_i, S).</li>
   * </ul>
   *
   ...
   *   a state rewriter
   * @param oracle
   *   an oracle that is created with newOracleWithDefault
   * @param set
   *   a set cell
   * @param setElems
   *   the cells pointed by the set
   */
