...
class FunExceptRule(rewriter: SymbStateRewriter) extends RewritingRule {
  private def cacheEq(s: SymbState, l: ArenaCell, r: ArenaCell) =
    rewriter.lazyEq.cacheOneEqConstraint(s, l, r)
    ...
    state.ex match {
      case OperEx(TlaFunOper.except, args @ _*) =>
        ...
        val (groundState: SymbState, groundArgs: Seq[TlaEx]) =
          rewriter.rewriteSeqUntilDone(
            state,
            funEx +: (unpackedIndices ++ valEs)
          )
        ...
        val valueCells = groundArgs
          .slice(
            1 + unpackedIndices.size,
            1 + unpackedIndices.size + valEs.size
          )
          .map(groundState.arena.findCellByNameEx)
        funCell.cellType match {
          case FunT(_, _) =>
            rewriteFun(groundState, funCell, indexCells, valueCells)
          case rt @ RecordT(_) =>
            rewriteRec(groundState, funCell, rt, unpackedIndices, valueCells)
          case tt @ TupleT(_) =>
            rewriteTuple(groundState, funCell, tt, unpackedIndices, valueCells)
          case _ =>
  ...
  def rewriteFun(
    ...
    def mkPair(indexCell: ArenaCell, resCell: ArenaCell): TlaEx =
      tla.tuple(indexCell.toNameEx, resCell.toNameEx)

    val (stateAfterTuples, updateTuples) =
      rewriter.rewriteSeqUntilDone(state, updatePairs map (mkPair _).tupled)
    val updateTuplesAsCells =
      updateTuples.map(stateAfterTuples.arena.findCellByNameEx(_))
    ...
    def eachRelationPair(p: ArenaCell): ArenaCell = {
      ...
      nextState =
        nextState.updateArena(_.appendHas(resultRelation, updatedCell))
      // the new cell belongs to the new relation iff the old cell belongs to the old relation
      solverAssert(
        tla.equiv(tla.in(p, relation), tla.in(updatedCell, resultRelation))
      )
      updatedCell
    }
  ...
  def rewriteRec(
    ...
    def indexToStr: TlaEx => String = {
      ...
      case ex =>
        throw new RewriterException(
          "Expected a string when updating a record, found: " + ex,
          ex
        )
    }
    ...
    // [ [k1, v1], [k2, v2], ... ]
    val updatedPairs: Seq[(TlaEx, TlaEx)] =
      indexEs.zip(valueCells.map(_.toNameEx))
    val unchangedPairs: Seq[(TlaEx, TlaEx)] =
      unchangedKeys.toList.map(mkUnchanged)
    val newRecEx = OperEx(
      TlaFunOper.enum,
      (updatedPairs ++ unchangedPairs).reverse
        .foldLeft(Seq[TlaEx]())(flattenPairs): _*
    )
  ...
  def rewriteTuple(
    ...
    def indexToInt: TlaEx => Int = {
      ...
      case ex =>
        throw new RewriterException(
          "Expected a number when updating a tuple, found: " + ex,
          ex
        )
    }
    ...
    def unpack(e: TlaEx) = e match {
      ...
      case OperEx(TlaFunOper.tuple, _*) =>
        throw new InternalCheckerError(
          "TLA importer failed to preprocess a chained EXCEPT: " + e,
          e
        )
      case _ =>
        // complain
        throw new RewriterException(
          "Expected a tuple as a function index, found: " + e,
          e
        )
    }
  ...
  private def checkType(cellType: CellT): Unit = {
    cellType match {
      ...
      case _ =>
        throw new NotImplementedError(
          s"EXCEPT is not implemented for $cellType. Write a feature request."
        )
    }
  }
}
