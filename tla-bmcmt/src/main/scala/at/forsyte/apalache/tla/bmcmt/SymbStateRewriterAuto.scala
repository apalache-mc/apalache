 ...
 * specifications should be processed by the special passes. However, it is much more convenient when all of this is
 * done automatically. This class does a lot of preprocessing, so it is not meant to be efficient at all. Use it only in
 * the unit tests.
 *
 * @author
 *   Igor Konnov
...
class SymbStateRewriterAuto(private var _solverContext: SolverContext, renaming: IncrementalRenaming)
  ...
  /**
   * Set the new solver context. Warning: the new context should be at the same stack depth as the rewriter. Otherwise,
   * pop may produce unexpected results.
   *
   * @param newContext
   *   new context
   */
