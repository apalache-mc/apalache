 ...
 *   Jure Kukovec
 */
class AndStratifiedRule(rewriter: Rewriter, shortCircuit: Boolean = false) extends StratifiedRule[Option[BuilderT]] {
      ...
      if (args.isEmpty) {
        ...
        if (shortCircuit) {
          ...
          val (newScope, rewrittenArgCells) =
            ...
            args.foldLeft((startingScope.copy(arena = arenaWithAndCell), Seq.empty[BuilderT])) {
