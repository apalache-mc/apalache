 ...
 * @param actionInvariantsAndNegations pairs of actions invariants and their negations.
 * @author Igor Konnov
 */
class CheckerInput(val rootModule: TlaModule, val initTransitions: List[TlaEx], val nextTransitions: List[TlaEx],
    val constInitPrimed: Option[TlaEx], val invariantsAndNegations: List[(TlaEx, TlaEx)],
    val actionInvariantsAndNegations: List[(TlaEx, TlaEx)]) {}
