 ...
 * @param index invariant index
 */
case class VCKind(index: Integer) extends ExprToLabelKind {}

/**
 * A cache for labels that are attached to expressions. The cache is used to avoid repeated calls of TlaExUtil.findLabels.
 */
class LabelsCache {
   ...
   * @param kind the kind of labels to look for (and cache)
   * @param expr the expression to search for labels
   * @return the sequence of labels of the given kind that are attached to the expression
   */
