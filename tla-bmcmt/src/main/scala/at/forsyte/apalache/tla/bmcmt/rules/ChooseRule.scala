...
class ChooseRule(rewriter: SymbStateRewriter) extends RewritingRule {
      ...
      case OperEx(TlaOper.chooseBounded, varName, set, pred) =>
        ...
        if (nextState.arena.getHas(setCell).isEmpty) {
          ...
          val (oracleState, oracle) =
            pickRule.oracleFactory.newDefaultOracle(nextState, elems.size + 1)
  ...
  private def happyChoose(
    ...
    val setCell = nextState.asCell
    val isFinSet = PartialFunction.cond(setCell.cellType) {
      case FinSetT(_) => true
    }
    if (isFinSet) {
  ...
  private def happyChooseFromFinite(
    ...
    if (state.arena.getHas(setCell).isEmpty) {
      ...
      nextState = rewriter.rewriteUntilDone(
        nextState.setRex(pred).setBinding(tempBinding)
      )
      solverAssert(nextState.ex)
      // return the witness
      nextState
        .setRex(witness.toNameEx)
        .setBinding(Binding(nextState.binding.toMap - varName))
    }
  }
  ...
  private def happyChooseFromOther(
    ...
    var nextState =
      pickRule.pick(setCell, state, state.arena.cellFalse().toNameEx)
    val witness = nextState.asCell
    // assert that the witness satisfies the predicate -- we are in the happy case
    val tempBinding = Binding(nextState.binding.toMap + (varName -> witness))
    nextState =
      rewriter.rewriteUntilDone(nextState.setRex(pred).setBinding(tempBinding))
    solverAssert(nextState.ex)
    // return the witness
    nextState
      .setRex(witness.toNameEx)
