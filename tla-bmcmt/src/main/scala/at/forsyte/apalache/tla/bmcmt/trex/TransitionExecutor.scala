...
/**
 * <p>A general-purpose symbolic transition executor (or T-Rex). It accumulates the basic logic for executing TLA+
 * transitions, which is used both in the sequential and parallel model checkers. (It could be used to implement
 * predicate abstraction too.) This class is imperative, as taking SMT snapshots and recovering them in the
 * non-incremental case is far from being functional.</p>
 *
 * <p>This class is parameterized by the type of an executor context. Currently, there are two choices: (1)
 * IncrementalExecutorContext and (2) OfflineExecutorContext.</p>
 *
 * @author
 *   Igor Konnov
 */
