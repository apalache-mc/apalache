...
class SymbStateDecoder(
  ...
  def dumpArena(state: SymbState, writer: PrintWriter): Unit = {
    ...
    def iseq(c: ArenaCell, d: ArenaCell): Boolean = {
      c.cellType == d.cellType && solverContext.evalGroundExpr(tla.eql(c, d)) == tla
        .bool(true)
    }

    def merge(
        cls: List[HashSet[ArenaCell]],
        c: ArenaCell,
        d: ArenaCell
    ): List[HashSet[ArenaCell]] = {
      if (!iseq(c, d) || c == d) {
        cls
      } else {
        ...
        val (two: List[HashSet[ArenaCell]], rest: List[HashSet[ArenaCell]]) =
          cls.partition(pred)
  ...
  def decodeStateVariables(state: SymbState): Map[String, TlaEx] = {
    state.binding.toMap.map(
      p =>
        (p._1, reverseMapVar(decodeCellToTlaEx(state.arena, p._2), p._1, p._2))
    )
  }

  def decodeCellToTlaEx(arena: Arena, cell: ArenaCell): TlaEx =
    cell.cellType match {
      ...
      case ConstT() =>
        ...
        if (found.isDefined) {
          ...
          findCellInSet(
            arena,
            rewriter.strValueCache.values().toSeq,
            cell.toNameEx
          ) match {
      ...
      case FinSetT(_) =>
        def inSet(e: ArenaCell) =
          solverContext.evalGroundExpr(tla.in(e, cell)) == tla.bool(true)
        ...
        val niceElems =
          decodedElems.distinct.sortWith(SymbStateDecoder.compareTlaExByStr)
        tla.enumSet(niceElems: _*)

      case FinFunSetT(_, _) =>
        tla.funSet(
          decodeCellToTlaEx(arena, arena.getDom(cell)),
          decodeCellToTlaEx(arena, arena.getCdm(cell))
        )
      ...
      case SeqT(_) =>
        ...
        startEndFun match {
          ...
          case _ =>
            throw new RewriterException(
              "Corrupted sequence: " + startEndFun,
              NullEx
            )
        }
      ...
      case r @ RecordT(_) =>
        def exToStr(ex: TlaEx): TlaStr = ex match {
          ...
          case _ =>
            throw new RewriterException("Expected a string, found: " + ex, ex)
        }
      ...
      case _ =>
        throw new RewriterException(
          "Don't know how to decode the cell %s of type %s"
            .format(cell, cell.cellType),
          NullEx
        )
    }
  ...
  private def decodeSet(arena: Arena, set: ArenaCell): Seq[TlaEx] = {
    decodeCellToTlaEx(arena, set) match {
      ...
      case _ =>
        throw new RewriterException(
          "Unexpected domain structure: " + set,
          NullEx
        )
    }
  }

  private def findCellInSet(
      arena: Arena,
      cells: Seq[ArenaCell],
      ex: TlaEx
  ): Option[ArenaCell] = {
    def isEq(c: ArenaCell): Boolean = {
      ValEx(TlaBool(true)) == solverContext.evalGroundExpr(
        tla.and(tla.eql(c, ex))
      )
    }
