...
object RecordingSolverContext {
  ...
  def create(
      ...
      config: SolverConfig,
      outputManager: Option[OutputManager] = None): RecordingSolverContext = {
    val solverImpl = SolverContextFactory.create(config, outputManager)
...
/**
 * A wrapper around SolverContext that records SMT operations. The log is organized hierarchically, so a differential
 * diff may be taken.
 *
 * @param parentLog
 *   the log of the parent context
 * @param config
...
class RecordingSolverContext private (
  ...
  /**
   * The sequence of logs (the last added element is in the head), one per context. Every element of stackOfInverseLogs
   * is stored in the reverse order.
   */
  ...
  /**
   * Declare a constant for an arena cell. This method is called automatically by the arena.
   *
   * @param cell
  ...
  /**
   * Declare an arena edge of type 'has'. This method introduces a Boolean variable for the edge. This method is called
   * automatically by the arena. If the context already contains a constant with the same name, then this method does
   * nothing.
   *
   * @param set
   *   the containing set
