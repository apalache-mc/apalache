...
class TrivialTypeFinder
    extends TypeFinder[CellT] with TransformationListener with Serializable with Recoverable[TrivialTypeSnapshot] {
      ...
      case OperEx(op, funEx, varsAndSets @ _*) if op == TlaFunOper.funDef || op == TlaFunOper.recFunDef =>
        ...
        def bind(name: String, set: TlaEx): Unit = {
          inferAndSave(set) match {
            ...
            case tp @ _ =>
              _varTypes =
                _varTypes + (name -> UnknownT()) // otherwise, the type rewriter throws an exception 10 lines below
              errorThenNone(set, "Expected a finite set, found: " + tp)
          }
        }
  ...
  private def computeSetOps(argTypes: Seq[CellT]): PartialFunction[TlaEx, CellT] = {
    ...
    case ex @ OperEx(op, _, _) if op == TlaSetOper.cup || op == TlaSetOper.cap || op == TlaSetOper.setminus =>
      argTypes match {
        case Seq(FinSetT(leftT), FinSetT(rightT)) =>
          expectEqualTypes(ex, leftT, rightT)
          FinSetT(leftT)
  ...
  private def computeIntOps(argTypes: Seq[CellT]): PartialFunction[TlaEx, CellT] = {
    ...
    case ex @ OperEx(op, _*) if op == TlaArithOper.sum || op == TlaArithOper.prod =>
      argTypes.foreach(expectType(IntT(), ex, _))
      IntT()
  }
  ...
  private def computeBoolOps(argTypes: Seq[CellT]): PartialFunction[TlaEx, CellT] = {
    ...
    case ex @ OperEx(op, _, _) if op == TlaBoolOper.implies || op == TlaBoolOper.equiv =>
      assert(argTypes.length == 2)
  ...
  private def computeFiniteSetOps(argTypes: Seq[CellT]): PartialFunction[TlaEx, CellT] = {
    case ex @ OperEx(op, _) if op == TlaFiniteSetOper.isFiniteSet || op == TlaFiniteSetOper.cardinality =>
      assert(argTypes.length == 1)
      argTypes.head match {
        case FinSetT(_) =>
  ...
  private def computeSeqOps(argTypes: Seq[CellT]): PartialFunction[TlaEx, CellT] = {
    case ex @ OperEx(op, _) if op == TlaSeqOper.head || op == TlaSeqOper.tail || op == TlaSeqOper.len =>
      assert(argTypes.length == 1)
      argTypes.head match {
        case SeqT(elemT) =>
  ...
  private def notImplemented: PartialFunction[TlaEx, CellT] = { case ex =>
    throw new NotImplementedError("Type construction for %s is not implemented. Report a bug!".format(ex))
  }
