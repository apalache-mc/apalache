...
object IncrementalRenaming {
    ...
    name.split(separatorRegex) match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "Variable names should never contain more than one separator"
        )
    }
  }
  ...
  /**
    ...
    */
  def mergeNameCounters(
      takeMax: Boolean
  )(m1: counterMapType, m2: counterMapType): counterMapType = {
  ...
  /**
    ...
    */
  def nameCounterMapFromEx(takeMax: Boolean)(ex: TlaEx): counterMapType =
    ex match {
      case NameEx(n) =>
        parseName(n) match {
      ...
      case OperEx(_, args @ _*) =>
        args
          .map(nameCounterMapFromEx(takeMax))
          .foldLeft(Map.empty[String, Int]) { mergeNameCounters(takeMax) }

      case LetInEx(body, defs @ _*) =>
        defs
          .map(nameCounterMapFromDecl(takeMax))
          .foldLeft(nameCounterMapFromEx(takeMax)(body)) {
  ...
  /**
    ...
    */
  def nameCounterMapFromDecl(takeMax: Boolean)(decl: TlaDecl): counterMapType =
    decl match {
      case TlaOperDecl(name, params, body) =>
        val nameAndParamMap =
          (parseName(name) ++: params.flatMap(p => parseName(p.name))).toMap
  ...
  /**
    ...
    */
  def nameCounterMapFromDecls(
      takeMax: Boolean
  )(decls: Traversable[TlaDecl]): counterMapType =
    decls
      .map(nameCounterMapFromDecl(takeMax))
      .foldLeft(Map.empty[String, Int]) { mergeNameCounters(takeMax) }
  ...
  /**
    ...
    */
  def offsetName(offsets: counterMapType)(name: String): String =
    parseName(name) match {
      case Some((base, ctr)) =>
...
/**
  ...
  */
@Singleton
class IncrementalRenaming @Inject()(tracker: TransformationTracker)
  ...
  private def rename(alreadyRenamed: Map[String, String]): TlaExTransformation =
    tracker.track {
      case ex @ NameEx(name) =>
      ...
      case OperEx(op, result, varsAndSets @ _*)
        ...
        val newRenamed = names.map(_.name).foldLeft(alreadyRenamed) {
          case (m, n) =>
        ...
        val newDefs = defs map {
          case TlaOperDecl(n, ps, b) =>
            TlaOperDecl(
              ...
              ps map {
                case SimpleFormalParam(p) =>
                  SimpleFormalParam(opersAndFParamsNameMap(p))
                case OperFormalParam(p, a) =>
                  OperFormalParam(opersAndFParamsNameMap(p), a)
              },
  ...
  def applyToDecls: Traversable[TlaDecl] => Traversable[TlaDecl] =
    Lift.declToDecls(applyToDecl)

  /**
    * Reduces all variable counters by the amount specified in offsets.
    */
  private[lir] def shiftCounters(offsets: counterMapType): TlaExTransformation =
    tracker.track {
      case ex @ NameEx(name) =>
  ...
  // lifted to decls
  private def shiftCountersDs(
      offsets: counterMapType
