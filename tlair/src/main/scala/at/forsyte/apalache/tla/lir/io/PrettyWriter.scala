...
/**
  ...
  * @author Igor Konnov
  */
class PrettyWriter(writer: PrintWriter, textWidth: Int = 80, indent: Int = 2)
  ...
  def toDoc(parentPrecedence: (Int, Int), expr: TlaEx): Doc = {
    expr match {
        ...
        lambdaStack match {
          case Nil =>
            throw new IllegalStateException(
              "Expected LAMBDA to be introduced earlier"
            )
      ...
      case OperEx(op @ TlaSetOper.enumSet, args @ _*) =>
        ...
        group(
          braces(group(softline <> nest(commaSeparated, indent)) <> softline)
        )
      ...
      case OperEx(op @ TlaFunOper.tuple, args @ _*) =>
        ...
        group(
          text("<<") <> nest(linebreak <> commaSeparated, indent) <> linebreak <> ">>"
        )
        ...
        if (args.isEmpty) {
          ...
          val grouped =
            if (argDocs.length <= 3) {
              ...
              val doc = nest(
                folddoc(argDocs, _ <> linebreak <> sign <> space <> _)
              )
        ...
        val boxes =
          keys
            ...
            .map(
              p =>
                group(
                  strNoQuotes(p._1) <> ":" <> nest(line <> toDoc((0, 0), p._2))
                )
            ) ///
      ...
      case OperEx(TlaSetOper.map, body, keysAndValues @ _*) =>
        ...
        val boxes =
          keys
            ...
            .map(
              p =>
                group(
                  toDoc(TlaSetOper.in.precedence, p._1) <> space <>
                    "\\in" <> nest(
                    line <> toDoc(TlaSetOper.in.precedence, p._2)
                  )
                )
            ) ///
        ...
        group(
          braces(
            nest(line <> bodyDoc <> text(":") <> nest(line <> binders)) <> line
          )
        )
      ...
      case OperEx(
        ...
        group(
          toDoc(TlaFunOper.app.precedence, funEx) <> brackets(
            commaSeparatedArgs
          )
        ) ///
      ...
      case OperEx(TlaFunOper.app, funEx, argEx) =>
        group(
          toDoc(TlaFunOper.app.precedence, funEx) <>
            text("[") <> nest(
            linebreak <> toDoc(TlaFunOper.app.precedence, argEx)
          ) <> linebreak <> text("]")
        ) ///
      ...
      case OperEx(
        ...
        val pairsWithOther =
          if (otherEx == NullEx) {
            ...
            pairs :+ group(
              "OTHER" <> nest(line <> "->" <> space <> toDoc(prec, otherEx))
            )
          }
      ...
      case OperEx(TlaControlOper.caseNoOther, guardsAndUpdates @ _*) =>
        ...
        toDoc(
          parentPrecedence,
          OperEx(TlaControlOper.caseWithOther, NullEx +: guardsAndUpdates: _*)
        )
      ...
      case OperEx(TlaFunOper.except, funEx, keysAndValues @ _*) =>
        ...
        val boxes =
          keys
            ...
            .map(
              p =>
                group(
                  text("!") <> brackets(toDoc((0, 0), p._1)) <> space <> text(
                    "="
                  ) <>
        ...
        val doc =
          text("[") <> nest(
            line <> toDoc(TlaFunOper.except.precedence, funEx) <>
              nest(softline <> text("EXCEPT") <> line <> updates)
          ) <> line <>
            text("]")
      ...
      case OperEx(op, arg @ OperEx(_, _))
          if PrettyWriter.unaryOps.contains(op) =>
      ...
      case LetInEx(body, decls @ _*) =>
        ...
        group(
          ssep(decls.map(eachDecl).toList, line) <>
            line <> toDoc((0, 0), body)
        )
    }
  }
  ...
  def toDoc(decl: TlaDecl): Doc = {
    decl match {
      ...
      case TlaOperDecl(
          name,
          List(),
          OperEx(TlaFunOper.recFunDef, body, keysAndValues @ _*)
          ) =>
        val (ks, vs) = keysAndValues.zipWithIndex partition (_._2 % 2 == 0)
        val (keys, values) = (ks.map(_._1), vs.map(_._1))
        // format each key-value pair (k, v) into k \in v
        ...
        // f[x \in S] == e
        val doc = group(
          name <> binding <> space <> "==" <> space <> toDoc((0, 0), body)
        )
  ...
  private def wrapWithParen(
      parentPrecedence: (Int, Int),
      exprPrecedence: (Int, Int),
      doc: Doc
  ): Doc = {
  ...
  private def strNoQuotes(ex: TlaEx): String = {
    ex match {
      ...
      case _ =>
        throw new IllegalStateException(
          "Expected a string as a record key, found: " + ex
        )
    }
  }
}
