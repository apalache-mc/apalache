 ...
 *
 * If a transformation takes ex1 with UID x as input and returns ex2 with UID y and x != y, then ChangeListener will
 * record y -> x. Moreover, if ex2 contains a subexpression g2 that is not tracked by ChangeListener (and none of its
 * ascendants but ex2 is tracked), then ChangeListener will record g2 -> ex1.
 *
 * TODO: Igor @ 11.08.2019, the name 'ChangeListener' sounds similar to 'TransformationListener'. Shall we give a better
 * name, for instance, PredIdSavingListener?
 *
 * TODO: Igor @ 11.08.2019, we are using Map[UID, UID] to record the changes between the subtrees. There must be a less
 * wasteful data structure for that.
 *
 * @author
 *   Jure Kukovec
 */
...
class ChangeListener extends TransformationListener {
   ...
   * @return
   *   true, if p_id has been registered
   */
