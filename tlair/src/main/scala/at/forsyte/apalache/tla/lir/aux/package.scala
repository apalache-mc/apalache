...
package object aux {
  ...
  def joinMaps(a: Map[String, Int], b: Map[String, Int]): Map[String, Int] =
    (for {
      x <- a.keySet.union(b.keySet)
    } yield x -> (a.getOrElse(x, 0) + b.getOrElse(x, 0))).toMap[String, Int]

  def countCandidates(vars: Set[String], ex: TlaEx): Map[String, Int] =
    ex match {
  ...
  /** We may need to split an ordered collection of OperDecls (from a LET-IN operator),
    ...
    */
  def collectSegments(
      decls: Traversable[TlaOperDecl]
  ): List[List[TlaOperDecl]] = decls match {
    ...
    case head :: tail =>
      ...
      if (recHeadOrEmpty.forall(d => hasPositiveArity(d) == headPosArity))
        (head +: recHeadOrEmpty) +: rec
          .drop(1) // Nil.tail throws, but Nil.drop(1) doesn't
      else
        List(head) +: rec
  }
  ...
  def diff(ex1: TlaEx, ex2: TlaEx): TlaEx = (ex1, ex2) match {
    case (OperEx(op1, args1 @ _*), OperEx(op2, args2 @ _*)) if op1 == op2 =>
      val argDiff = args1.zipAll(args2, NullEx, NullEx) map {
        case (x, y) => diff(x, y)
      }
    ...
    case (LetInEx(b1, decls1 @ _*), LetInEx(b2, decls2 @ _*)) =>
      ...
      val declDiff = decls1.zipAll(decls2, defaultDecl, defaultDecl) map {
        case (d1, d2) =>
          if (d1.name == d2.name && d1.formalParams == d2.formalParams)
            ...
            val params = d1.formalParams
              .zipAll(d2.formalParams, defaultParam, defaultParam) map {
              case (par1 @ SimpleFormalParam(p1), SimpleFormalParam(p2))
                  if p1 == p2 =>
                par1
              case (par1 @ OperFormalParam(p1, n1), OperFormalParam(p2, n2))
                  if p1 == p2 && n1 == n2 =>
                par1
              case (par1, par2) =>
