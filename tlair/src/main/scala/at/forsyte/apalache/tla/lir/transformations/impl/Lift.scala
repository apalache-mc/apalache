...
object Lift {
  ...
  def declToDecls( tr: TlaDecl => TlaDecl ) : Traversable[TlaDecl] => Traversable[TlaDecl] =
    _ map tr

  def exToDecls( tr : TlaExTransformation ) : Traversable[TlaDecl] => Traversable[TlaDecl] =
    _ map exToDecl( tr )
}
