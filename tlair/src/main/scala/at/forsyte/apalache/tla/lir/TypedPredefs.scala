...
object TypedPredefs {
  ...
  /**
   * An implicit conversion of an expression to a typed builder expression. We need this implicit conversion in addition
   * to tlaExToBlock, as Scala looks only for one implicit conversion, whereas we need two in this case.
   *
   * @param ex
   *   a TLA+ expression
   * @return
   *   a typed builder block
   */
  ...
  implicit class TypeTagAsTlaType1(tag: TypeTag) {
    ...
    /**
     * Unwrap a type tag of the form Typed(_: TlaType1) into TlaType1. If the tag does not match this pattern, throw
     * TypingException.
     *
     * @return
     *   the type that is stored in the tag
     */
  ...
  implicit class BuilderExAsTyped(block: BuilderEx) {
     ...
     *
     * @param topType
     *   the type to use for the top expression
     * @return
     *   an expression with the given type
     */
