 ...
 *   the error message
 * @param failedIds
 *   the identified of the expressions that caused trouble
 */
 ...
 *   the error message
 * @param causeModule
 *   the module that lacks some transformation
 */
...
/**
 * This exception is thrown when an input specification is malformed at the declaration or module level. For malformed
 * expressions, use MalformedTlaException
 *
 * @param message
 *   the error message
 */
...
/**
 * An exception that should be thrown when a user-defined operator calls itself via a chain of calls to other operators
 * (in a non-recursive case).
 *
 * @param message
 *   the error message
 */
 ...
 *   error message
 * @param causeExprId
 *   the id of a problematic expression, may be UID.nullId
 */
