...
object JsonReader {
  ...
  def parseModule(v: ujson.Value): TlaModule = {
    ...
    val m = v.objOpt match {
      ...
      case None =>
        throw new Exception("incorrect TLA+ JSON: expecting a module object")
    }
  ...
  val otherOps =
    Set("id", "str", "int", "set", "applyFun", "applyOp", "if", "case", "let")
    ...
    v match {
      ...
      case ujson.Num(value) =>
        try {
          ...
          val decimal = BigDecimal(value.toString.toDouble)
  ...
  def parseExpr(m: LinkedHashMap[String, ujson.Value]): TlaEx = {
    ...
    val expr =
      if (ourKeys < 1)
        throw new Exception(
          "incorrect TLA+ JSON: expected expression, but none found"
        )
      else if (ourKeys > 1)
        throw new Exception(
          "incorrect TLA+ JSON: multiple matching expressions"
        )
        ...
        OperEx(
          binaryOps(binary.head),
          parseJson(m(binary.head)),
          parseJson(m("arg"))
        )
        ...
        OperEx(
          functionalOps(functional.head),
          parseJson(m(functional.head)) +: parsePairs(m("where")): _*
        )
      } else if (unboundedPred.nonEmpty) {
        if (!m.contains("that"))
          throw new Exception("incorrect TLA+ JSON: expecting 'that'")
        OperEx(
          unboundedPredOps(unboundedPred.head),
          parseJson(m(unboundedPred.head)),
          parseJson(m("that"))
        )
        ...
        if (!m.contains("that"))
          throw new Exception("incorrect TLA+ JSON: expecting 'that'")
        OperEx(
          boundedPredOps(boundedPred.head),
          nameSet(0),
          nameSet(1),
          parseJson(m("that"))
        )
        ...
        OperEx(
          stutterOps(stutter.head),
          parseJson(m(stutter.head)),
          parseJson(m("vars"))
        )
        ...
        if (!m.contains("vars"))
          throw new Exception("incorrect TLA+ JSON: expecting 'vars'")
        OperEx(
          fairnessOps(fairness.head),
          parseJson(m("vars")),
          parseJson(m(fairness.head))
        )
        ...
        other.head match {
          ...
          case "set" => {
            ...
            if (sets.contains(set))
              ...
              throw new Exception(
                "can't parse TLA+ JSON: reference to unknown set"
              )
          }
          case "applyFun" => {
            if (!m.contains("arg"))
          ...
          case "applyOp" => {
            ...
            if (name == "recFunRef") {
              if (args.nonEmpty)
                throw new Exception(
                  "incorrect TLA+ JSON: found arguments for 'recFunRef'"
                )
          ...
          case "case" => {
            if (m.contains("other"))
              OperEx(
                ...
                parseJson(m("other")) +: parsePairs(m("case")): _*
              )
            else
        ...
        throw new Exception(
          "can't parse TLA+ JSON: cannot find a known JSON key"
        )
    if (m.contains("label")) {
  ...
  def parseArray(v: ujson.Value): Seq[TlaEx] = {
    ...
    val arr = v.arrOpt match {
      case Some(value) => value
      case None =>
        throw new Exception("incorrect TLA+ JSON: expecting expression array")
    }
    arr.map(parseJson)
  }
  ...
  def parsePairs(v: ujson.Value): Seq[TlaEx] = {
    ...
    val arr = v.arrOpt match {
      case Some(value) => value
      case None =>
        throw new Exception("incorrect TLA+ JSON: expecting array of pairs")
    }
  ...
  def parsePair(v: ujson.Value): Seq[TlaEx] = {
    val m = v.objOpt match {
      case Some(value) => value
      case None =>
        throw new Exception("incorrect TLA+ JSON: expecting a key-value object")
    }
  ...
  def parseLabel(v: ujson.Value): (String, Seq[TlaEx]) = {
    ...
    val m = v.objOpt match {
      case Some(value) => value
      case None =>
        throw new Exception("incorrect TLA+ JSON: expecting a label object")
    }
  ...
  def parseDecls(v: ujson.Value): Seq[TlaDecl] = {
    ...
    val arr = v.arrOpt match {
      case Some(value) => value
      case None =>
        throw new Exception("incorrect TLA+ JSON: expecting declaration array")
    }
    arr.map(parseDecl)
  }
  ...
  def parseDecl(v: ujson.Value): TlaDecl = {
    val m = v.objOpt match {
      ...
      case None =>
        throw new Exception(
          "incorrect TLA+ JSON: expecting a declaration object"
        )
    }
    if (m.contains("constant"))
      ...
      if (!m.contains("body") || !m.contains("params"))
        throw new Exception(
          "incorrect TLA+ JSON: malformed operator declaration"
        )
      TlaOperDecl(
        parseStr(m("operator")),
        parseParams(m("params")).toList,
        parseJson(m("body"))
      )
    } else
  ...
  def parseParams(v: ujson.Value): Seq[FormalParam] = {
    ...
    val arr = v.arrOpt match {
      case Some(value) => value
      case None =>
        throw new Exception("incorrect TLA+ JSON: expecting parameter array")
    }
    arr.map(parseParam)
  }
  ...
  def parseParam(v: ujson.Value): FormalParam = {
    ...
    val m = v.objOpt match {
      case Some(value) => value
      case None =>
        throw new Exception("incorrect TLA+ JSON: expecting a parameter object")
    }
