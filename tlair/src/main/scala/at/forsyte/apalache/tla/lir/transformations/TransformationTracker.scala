...
trait TransformationTracker {
   ...
   * @return
   *   a new transformation that applies tr and tracks the changes
   */
  def trackEx(tr: TlaExTransformation): TlaExTransformation

  /**
   * Decorate a transformation with a tracker. By convention, the tracker is only tracking transformations that produce
   * Right[TlaEx]
   *
   * @param tr
   *   an expression transformation
   * @return
   *   a new transformation that applies tr and tracks the changes if the transformation is going via the constructor
   *   Right.
   */
   ...
   *
   * @param tr
   *   a declaration transformation
   * @return
   *   a new declaration transformation that applies tr and tracks the changes
   */
  def trackDecl(tr: TlaDeclTransformation): TlaDeclTransformation

  /**
   * A specialized version of trackDecl, which is tuned to TlaOperDecl. Most transformation apply only to user-defined
   * operators. That is why we have added a specialized method for it.
   *
   * @param tr
   *   a declaration transformation
   * @return
  ...
  /**
   * Sometimes, one has to track a change in a temporary expression that will get transformed into something else before
   * it gets a chance to get tracked with `track`, which only works with end-to-end transformations. In this case, we
   * have to record the change as a side effect and return the new expression. Avoid using this method by default. This
   * is a second-chance method.
   *
   * @param from
   ...
   * <ol> <li>Copies the declaration body and applies the expression transformation to it, and</li> <li>Tracks the
   * update of the declaration.</li>
   *
  ...
  def fromExToDeclTransformation(
      transformation: TlaExTransformation): TlaDeclTransformation =
    trackDecl {
