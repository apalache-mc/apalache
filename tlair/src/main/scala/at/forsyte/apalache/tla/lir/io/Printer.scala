...
object UTFPrinter extends Printer {
  ...
  def apply(p_ex: TlaEx, p_rmSpace: Boolean): String = {
    def mapMk(seq: Seq[TlaEx], sep: String = ", ", fn: TlaEx => String) =
    ...
    def opApp(p_ex: TlaEx): String = {
      p_ex match {
        case OperEx(TlaSetOper.enumSet | TlaSetOper.in | TlaFunOper.app, _*) =>
          apply(p_ex)
    ...
    def mkApp(formatString: String, args: TlaEx*) =
      formatString.format(args.map(apply): _*)

    def mkOpApp(formatString: String, args: TlaEx*) =
      formatString.format(args.map(opApp): _*)
    ...
    val ret = p_ex match {
      ...
      case OperEx(oper, args @ _*) =>
        oper match {
          ...
          case TlaOper.apply =>
            "%s(%s)".format(opApp(args.head), str(args.tail))
          case TlaOper.chooseUnbounded => mkOpApp("CHOOSE %s : %s", args: _*)
          case TlaOper.chooseBounded =>
            mkOpApp("CHOOSE %%s %s %%s : %%s".format(m_in), args: _*)
          ...
          case TlaBoolOper.implies =>
            mkOpApp("%%s %s %%s".format(m_impl), args: _*)
          case TlaBoolOper.equiv =>
            mkOpApp("%%s %s %%s".format(m_equiv), args: _*)
          case TlaBoolOper.forall =>
            mkOpApp("%s%%s %s %%s: %%s".format(m_forall, m_in), args: _*)
          case TlaBoolOper.exists =>
            mkOpApp("%s%%s %s %%s: %%s".format(m_exists, m_in), args: _*)
          case TlaBoolOper.forallUnbounded =>
            mkOpApp("%s%%s: %%s".format(m_forall), args: _*)
          case TlaBoolOper.existsUnbounded =>
            mkOpApp("%s%%s: %%s".format(m_exists), args: _*)
          ...
          case TlaActionOper.composition =>
            mkOpApp("%%s %s %%s".format(m_cdot), args: _*)

          case TlaControlOper.caseNoOther =>
            "CASE %s".format(opAppStrPairs(args))
          case TlaControlOper.caseWithOther =>
            "CASE %s %s OTHER %s %s".format(
              opAppStrPairs(args.tail),
              m_box,
              m_rarrow,
              args.head
            )
          case TlaControlOper.ifThenElse =>
            mkOpApp("IF %s THEN %s ELSE %s", args: _*)

          case TlaTempOper.AA =>
            mkOpApp("[%s]%%s . %%s".format(m_forall), args: _*)
          case TlaTempOper.box => mkOpApp("%s%%s".format(m_box), args: _*)
          case TlaTempOper.diamond =>
            mkOpApp("%s%%s".format(m_diamond), args: _*)
          case TlaTempOper.EE =>
            mkOpApp("[%s]%%s . %%s".format(m_exists), args: _*)
          case TlaTempOper.guarantees =>
            mkOpApp("%%s %s %%s".format(m_guarantee), args: _*)
          case TlaTempOper.leadsTo =>
            mkOpApp("%%s %s %%s".format(m_leadsto), args: _*)
          ...
          case TlaFiniteSetOper.cardinality =>
            mkApp("Cardinality(%s)", args: _*)
          case TlaFiniteSetOper.isFiniteSet =>
            mkApp("IsFiniteSet(%s)", args: _*)

          case TlaFunOper.app =>
            "%s[%s]".format(opApp(args.head), apply(args.tail.head))
          case TlaFunOper.domain => mkOpApp("DOMAIN %s", args: _*)
          case TlaFunOper.enum =>
            "[%s]".format(opAppStrPairs(args, pad(m_mapto), ", "))
          case TlaFunOper.except =>
            "[%s EXCEPT %s]".format(
              apply(args.head),
              opAppPattern(args.tail, 2, "![%s] = %s", ", ")
            )
          case TlaFunOper.funDef =>
            "[%s %s %s]".format(
              opAppStrPairs(args.tail, pad(m_in), ", "),
              m_mapto,
              apply(args.head)
            )
          ...
          case TlaSeqOper.concat =>
            mkOpApp("%%s %s %%s".format(m_ring), args: _*)
          ...
          case TlaSetOper.notin =>
            mkOpApp("%%s %s %%s".format(m_notin), args: _*)
          ...
          case TlaSetOper.filter =>
            mkOpApp("{%%s %s %%s: %%s}".format(m_in), args: _*)
          case TlaSetOper.funSet =>
            mkOpApp("[%%s %s %%s]".format(m_rarrow), args: _*)
          case TlaSetOper.map =>
            "{%s : %s}".format(
              apply(args.head),
              opAppStrPairs(args.tail, pad(m_in), ", ")
          ...
          case TlaSetOper.setminus =>
            mkOpApp("%%s %s %%s".format(m_setminus), args: _*)
          case TlaSetOper.subseteq =>
            mkOpApp("%%s %s %%s".format(m_subseteq), args: _*)
          case TlaSetOper.subsetProper =>
            mkOpApp("%%s %s %%s".format(m_subset), args: _*)
          case TlaSetOper.supseteq =>
            mkOpApp("%%s %s %%s".format(m_supseteq), args: _*)
          case TlaSetOper.supsetProper =>
            mkOpApp("%%s %s %%s".format(m_supset), args: _*)
  ...
  override def apply(p_decl: TlaDecl): String = {
    def pr_param(p: FormalParam): String = {
      ...
      val params =
        if (arity == 0) ""
        else "(%s)".format(1.to(arity).map(_ => "_").mkString(", "))
      p.name + params
    }
...
object SimplePrinter extends Printer {
  ...
  override def apply(p_ex: TlaEx): String =
    UTFPrinter.apply(p_ex).map(c => getSimple(c.toString)).mkString
}
