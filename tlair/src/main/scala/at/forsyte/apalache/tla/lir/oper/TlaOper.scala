...
trait TlaOper extends Serializable {
   ...
   * @return
   *   true, if this number of arguments is allowed.
   */
  def isCorrectArity(a: Int): Boolean = arity.cond(a)

  /**
   * Do the operator arguments satisfy the operator invariant? For instance, some operators only allow name expressions
...
object TlaOper {
   ...
   * @return
   *   a pair of lists: on even positions and on odd positions
   */
   ...
   *
   * @param even
   *   the list of even arguments
   * @param odd
   *   the list of odd arguments
   * @return
  ...
  /**
   * <p>Operator application by name, e.g, <code>OperEx(apply, f, x, y)</code> calls <code>f(x, y)</code>. This operator
   * is similar to a function call in the programming languages. </p>
   *
   * <p>This is an operator that you will not find in TLA+ code. This operator appears in IR in two cases: (1) when a
   * user-defined operator is called, either defined with a top-level definition or LET-IN, and (2) when a parameter of
   * a user-defined operator is an operator itself, and it is applied to an argument.</p>
   *
   * <p> Examples: <ol> <li>Consider two top-level definitions:
   *
   * <pre> A(i) == i + 1 B(j) == A(j) </pre>
   *
   * The body of B is represented as <code>OperEx(apply, NameEx("A"), NameEx("j"))</code>
   *
   * </li> <li>Consider a top-level definition:
   *
   * <pre> A(f(_), i) == f(i) </pre>
   *
   * The body of A is represented as <code>OperEx(apply, NameEx("f"), NameEx("i"))</code> </li> </ol> </p>
   *
   * @see
   *   TestSanyImporter.test("level-2 operators")
   */
  ...
  /**
   * <p>An operator that decorates an expression with a label, e.g., l3(a, b) :: ex. The order of the arguments is as
   * follows: (1) the decorated expression, e.g., ex, (2) the label, e.g., ValEx(TlaStr("l3")), and (3 to k) the label
   * arguments, which must be strings, e.g., ValEx(TlaStr("a")) and ValEx(TlaStr("b")).</p>
   *
   * <p>To get more info about labels, see <a
   * href="http://research.microsoft.com/en-us/um/people/lamport/tla/tla2-guide.pdf">TLA+2 Preliminary Guide</a>.</p>
   *
   * <p>Technically, a label is not an operator in TLA+, but a special form of an expression. As the labels are rarely
   * used, we have decided to introduce a new operator instead of extending TlaEx with a new case class (it would have
   * been annoying to take care of LabelEx in the pattern-matching code).</p>
   */
  object label extends TlaControlOper {
     ...
     * expressions in certain positions.
     *
     * @param args
     *   operator arguments
     * @return
