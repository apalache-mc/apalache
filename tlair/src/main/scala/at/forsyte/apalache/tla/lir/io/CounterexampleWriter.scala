...
class TlaCounterexampleWriter(writer: PrintWriter)
  ...
  override def write(
    ...
    writer.println("\n\n%s".format("=" * 80))
    writer.println(
      "\\* Created by Apalache on %s".format(Calendar.getInstance().getTime)
    )
...
class TlcCounterexampleWriter(writer: PrintWriter)
    extends TlaCounterexampleWriter(writer) {
...
class JsonCounterexampleWriter(writer: PrintWriter)
    extends CounterexampleWriter {
  override def write(
      rootModule: TlaModule,
      notInvariant: NotInvariant,
    ...
    val tlaStates = states.zipWithIndex.collect {
      case ((tran, vars), i) =>
        ...
        TlaOperDecl(
          s"State${i + 1}",
          List(),
          OperEx(TlaBoolOper.and, state.toList: _*)
        )
    ...
    val mod = new TlaModule(
      "counterexample",
      tlaStates ++ List(
        TlaOperDecl(s"InvariantViolation", List(), notInvariant)
      )
    )
    json.write(mod)
  }
