...
/**
 * Handles the identification of special model value strings. These values are input as strings, but their types (and
 * SMT sorts) are unique and they are incomparable with regular strings of with other model values of a different type.
 *
...
object ModelValueHandler {
  ...
  /**
   * Attempts to extract the type and index from thew pattern. If None, the value `s` is not a model value, but a
   * regular string.
   */
