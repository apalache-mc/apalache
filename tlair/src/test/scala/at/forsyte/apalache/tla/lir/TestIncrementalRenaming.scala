...
class TestIncrementalRenaming
  ...
  test("Test apply: basic") {
    ...
    assert(
      renamed2 == exists(name(makeName("s", 1)), n_T, name(makeName("s", 1)))
    )

    val renamed2again = renaming(renamed2)
    assert(
      renamed2again == exists(
        name(makeName("s", 2)),
        n_T,
        name(makeName("s", 2))
      )
  ...
  test("Test apply: exists/forall") {
    ...
    def expected(offset: Int): TlaEx =
      and(
        exists(
          name(makeName("x", offset + 1)),
          n_S,
          gt(name(makeName("x", offset + 1)), int(1))
        ),
        forall(
          name(makeName("x", offset + 2)),
          n_T,
          lt(name(makeName("x", offset + 2)), int(42))
        )
      )
    ...
    def expected(offset: Int): TlaEx =
      cup(
        filter(
          name(makeName("x", offset + 1)),
          n_S,
          eql(name(makeName("x", offset + 1)), int(1))
        ),
        filter(
          name(makeName("x", offset + 2)),
          n_S,
          eql(name(makeName("x", offset + 2)), int(2))
        )
      )
    ...
    val expected =
      letIn(
        exists(
          name(makeName("x", 2)),
          n_S,
          appOp(name(makeName("p", 1)), name(makeName("x", 2)))
        ),
        declOp(
          makeName("p", 1),
          forall(
            name(makeName("x", 1)),
            n_S,
            appOp(name("R"), name(makeName("t", 1)), name(makeName("x", 1)))
          ),
          makeName("t", 1)
        )
      )
    ...
    val expected2 =
      letIn(
        exists(
          name(makeName("x", 4)),
          n_S,
          appOp(name(makeName("p", 2)), name(makeName("x", 4)))
        ),
        declOp(
          makeName("p", 2),
          forall(
            name(makeName("x", 3)),
            n_S,
            appOp(name("R"), name(makeName("t", 2)), name(makeName("x", 3)))
          ),
          makeName("t", 2)
        )
      )
