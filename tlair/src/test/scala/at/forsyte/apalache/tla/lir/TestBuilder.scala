...
class TestBuilder extends FunSuite with TestingPredefs {
  ...
  test("Test byName: bad calls") {
    assertThrows[NoSuchElementException](
      bd.byName("not an operator name", NullEx, n_b)
    )

    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.plus.name, NullEx)
    )
  }
  ...
  test("Test byName: TlaOper") {
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaOper.eq.name, n_a, n_b, n_c)
    )
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaOper.ne.name, n_a, n_b, n_c)
    )

    val cbBuild = bd.byName(TlaOper.chooseBounded.name, n_a, n_b, n_c)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaOper.chooseBounded.name, n_a, n_b)
    )
    assert(cbBuild == OperEx(TlaOper.chooseBounded, n_a, n_b, n_c))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaOper.chooseBounded.name, n_a, n_b, n_c, n_d)
    )
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaOper.chooseUnbounded.name, n_a, n_b, n_c)
    )
    assertThrows[IllegalArgumentException](
      bd.byName(TlaOper.chooseUnbounded.name, n_a, n_b, n_c, n_d)
    )
  }
  ...
  test("Test byNameOrNull: TlaOper") {
    ...
    val cbBuildBad2 =
      bd.byNameOrNull(TlaOper.chooseBounded.name, n_a, n_b, n_c, n_d)
    ...
    val cubBuildBad1 =
      bd.byNameOrNull(TlaOper.chooseUnbounded.name, n_a, n_b, n_c)
    val cubBuildBad2 =
      bd.byNameOrNull(TlaOper.chooseUnbounded.name, n_a, n_b, n_c, n_d)
  ...
  test("Test byName: TlaBoolOper ") {
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaBoolOper.not.name, n_a, n_b)
    )

    val impliesBuild = bd.byName(TlaBoolOper.implies.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaBoolOper.implies.name, n_a)
    )
    assert(impliesBuild == OperEx(TlaBoolOper.implies, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaBoolOper.implies.name, n_a, n_b, n_c)
    )
  ...
  test("Test byNameOrNull: TlaBoolOper ") {
    ...
    val impliesBuildBad2 =
      bd.byNameOrNull(TlaBoolOper.implies.name, n_a, n_b, n_c)
  ...
  test("Test direct methods: TlaActionOper") {
    ...
    assert(
      primeEqBuild2 == OperEx(TlaOper.eq, OperEx(TlaActionOper.prime, n_a), n_b)
    )
    assert(
      primeEqBuild3 == OperEx(
        TlaOper.eq,
        OperEx(TlaActionOper.prime, NameEx("name")),
        ValEx(TlaInt(1))
      )
    ...
    assert(
      primeEqBuild4 == OperEx(
        TlaOper.eq,
        OperEx(TlaActionOper.prime, n_a),
        ValEx(TlaInt(2))
      )
    )
    assert(
      primeEqBuild5 == OperEx(
        TlaOper.eq,
        OperEx(TlaActionOper.prime, NameEx("name1")),
        NameEx("name2")
      )
    )
  ...
  test("Test byName: TlaActionOper") {
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.prime.name, n_a, n_b)
    )

    val stutterBuild = bd.byName(TlaActionOper.stutter.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.stutter.name, n_a)
    )
    assert(stutterBuild == OperEx(TlaActionOper.stutter, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.stutter.name, n_a, n_b, n_c)
    )

    val nostutterBuild = bd.byName(TlaActionOper.nostutter.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.nostutter.name, n_a)
    )
    assert(nostutterBuild == OperEx(TlaActionOper.nostutter, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.nostutter.name, n_a, n_b, n_c)
    )

    val enabledBuild = bd.byName(TlaActionOper.enabled.name, n_a)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.enabled.name)
    )
    assert(enabledBuild == OperEx(TlaActionOper.enabled, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.enabled.name, n_a, n_b)
    )

    val unchangedBuild = bd.byName(TlaActionOper.unchanged.name, n_a)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.unchanged.name)
    )
    assert(unchangedBuild == OperEx(TlaActionOper.unchanged, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.unchanged.name, n_a, n_b)
    )

    val compositionBuild = bd.byName(TlaActionOper.composition.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.composition.name, n_a)
    )
    assert(compositionBuild == OperEx(TlaActionOper.composition, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaActionOper.composition.name, n_a, n_b, n_c)
    )
  }
  ...
  test("Test byNameOrNull: TlaActionOper") {
    ...
    val stutterBuildBad2 =
      bd.byNameOrNull(TlaActionOper.stutter.name, n_a, n_b, n_c)
    ...
    val nostutterBuildBad2 =
      bd.byNameOrNull(TlaActionOper.nostutter.name, n_a, n_b, n_c)
    ...
    val unchangedBuildBad2 =
      bd.byNameOrNull(TlaActionOper.unchanged.name, n_a, n_b)

    assert(unchangedBuildBad1 == NullEx)
    assert(unchangedBuild == OperEx(TlaActionOper.unchanged, n_a))
    assert(unchangedBuildBad2 == NullEx)

    val compositionBuildBad1 =
      bd.byNameOrNull(TlaActionOper.composition.name, n_a)
    val compositionBuild =
      bd.byNameOrNull(TlaActionOper.composition.name, n_a, n_b)
    val compositionBuildBad2 =
      bd.byNameOrNull(TlaActionOper.composition.name, n_a, n_b, n_c)
  ...
  test("Test direct methods: TlaControlOper") {
    ...
    assert(
      caseSplitBuild2 == OperEx(TlaControlOper.caseNoOther, n_a, n_b, n_c, n_d)
    )
    assertThrows[IllegalArgumentException](
      bd.caseSplit(n_a, n_b, n_c, n_d, n_e)
    )
    assert(
      caseSplitBuild3 == OperEx(
        TlaControlOper.caseNoOther,
        n_a,
        n_b,
        n_c,
    ...
    assert(
      caseOtherBuild1 == OperEx(TlaControlOper.caseWithOther, n_a, n_b, n_c)
    )
    assertThrows[IllegalArgumentException](bd.caseOther(n_a, n_b, n_c, n_d))
    assert(
      caseOtherBuild2 == OperEx(
        TlaControlOper.caseWithOther,
        n_a,
        n_b,
        n_c,
    ...
    assertThrows[IllegalArgumentException](
      bd.caseOther(n_a, n_b, n_c, n_d, n_e, n_f)
    )
    assert(
      caseOtherBuild3 == OperEx(
        TlaControlOper.caseWithOther,
        n_a,
        n_b,
        n_c,
  ...
  test("Test byName: TlaControlOper") {
    ...
    val caseBuild3 =
      bd.byName(TlaControlOper.caseNoOther.name, n_a, n_b, n_c, n_d)
    val caseBuild4 =
      bd.byName(TlaControlOper.caseWithOther.name, n_a, n_b, n_c, n_d, n_e)
    val caseBuild5 =
      bd.byName(TlaControlOper.caseNoOther.name, n_a, n_b, n_c, n_d, n_e, n_f)
    val caseBuild6 = bd.byName(
      TlaControlOper.caseWithOther.name,
      n_a,
      n_b,
      n_c,
      n_d,
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaControlOper.caseNoOther.name)
    )
    assert(caseNoOtherBuild == OperEx(TlaControlOper.caseNoOther, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaControlOper.caseNoOther.name, n_a, n_b, n_c)
    )

    val caseWithOtherBuild =
      bd.byName(TlaControlOper.caseWithOther.name, n_a, n_b, n_c)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaControlOper.caseWithOther.name)
    )
    assertThrows[IllegalArgumentException](
      bd.byName(TlaControlOper.caseWithOther.name, n_a)
    )
    assertThrows[IllegalArgumentException](
      bd.byName(TlaControlOper.caseWithOther.name, n_a, n_b)
    )
    assert(
      caseWithOtherBuild == OperEx(TlaControlOper.caseWithOther, n_a, n_b, n_c)
    )

    val iteBuild = bd.byName(TlaControlOper.ifThenElse.name, n_a, n_b, n_c)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaControlOper.ifThenElse.name, n_a, n_b)
    )
    assert(iteBuild == OperEx(TlaControlOper.ifThenElse, n_a, n_b, n_c))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaControlOper.ifThenElse.name, n_a, n_b, n_c, n_d)
    )
  }
  ...
  test("Test byNameOrNull: TlaControlOper") {
    ...
    val caseBuild2 =
      bd.byNameOrNull(TlaControlOper.caseWithOther.name, n_a, n_b, n_c)
    val caseBuild3 =
      bd.byNameOrNull(TlaControlOper.caseNoOther.name, n_a, n_b, n_c, n_d)
    val caseBuild4 = bd.byNameOrNull(
      TlaControlOper.caseWithOther.name,
      n_a,
      n_b,
      n_c,
    ...
    val caseBuild5 = bd.byNameOrNull(
      TlaControlOper.caseNoOther.name,
      n_a,
      n_b,
      n_c,
      n_d,
    ...
    val caseBuild6 = bd.byNameOrNull(
      TlaControlOper.caseWithOther.name,
      n_a,
      n_b,
      n_c,
    ...
    val caseNoOtherBuild =
      bd.byNameOrNull(TlaControlOper.caseNoOther.name, n_a, n_b)
    val caseNoOtherBuildBad =
      bd.byNameOrNull(TlaControlOper.caseNoOther.name, n_a, n_b, n_c)

    assert(caseNoOtherBuildEmpty == NullEx)
    assert(caseNoOtherBuild == OperEx(TlaControlOper.caseNoOther, n_a, n_b))
    assert(caseNoOtherBuildBad == NullEx)

    val caseWithOtherBuildEmpty =
      bd.byNameOrNull(TlaControlOper.caseWithOther.name)
    val caseWithOtherBuildSingle =
      bd.byNameOrNull(TlaControlOper.caseWithOther.name, n_a)
    val caseWithOtherBuildBad =
      bd.byNameOrNull(TlaControlOper.caseWithOther.name, n_a, n_b)
    val caseWithOtherBuild =
      bd.byNameOrNull(TlaControlOper.caseWithOther.name, n_a, n_b, n_c)
    ...
    val iteBuild =
      bd.byNameOrNull(TlaControlOper.ifThenElse.name, n_a, n_b, n_c)
    val iteBuildBad2 =
      bd.byNameOrNull(TlaControlOper.ifThenElse.name, n_a, n_b, n_c, n_d)
  ...
  test("Test byName: TlaTempOper") {
    ...
    assert(AABuild == OperEx(TlaTempOper.AA, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.AA.name, n_a, n_b, n_c)
    )
    ...
    assert(EEBuild == OperEx(TlaTempOper.EE, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.EE.name, n_a, n_b, n_c)
    )
    ...
    assert(boxBuild == OperEx(TlaTempOper.box, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.box.name, n_a, n_b)
    )
    ...
    assert(diamondBuild == OperEx(TlaTempOper.diamond, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.diamond.name, n_a, n_b)
    )

    val leadsToBuild = bd.byName(TlaTempOper.leadsTo.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.leadsTo.name, n_a)
    )
    assert(leadsToBuild == OperEx(TlaTempOper.leadsTo, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.leadsTo.name, n_a, n_b, n_c)
    )

    val guaranteesBuild = bd.byName(TlaTempOper.guarantees.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.guarantees.name, n_a)
    )
    assert(guaranteesBuild == OperEx(TlaTempOper.guarantees, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.guarantees.name, n_a, n_b, n_c)
    )

    val strongFairnessBuild =
      bd.byName(TlaTempOper.strongFairness.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.strongFairness.name, n_a)
    )
    assert(strongFairnessBuild == OperEx(TlaTempOper.strongFairness, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.strongFairness.name, n_a, n_b, n_c)
    )

    val weakFairnessBuild = bd.byName(TlaTempOper.weakFairness.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.weakFairness.name, n_a)
    )
    assert(weakFairnessBuild == OperEx(TlaTempOper.weakFairness, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaTempOper.weakFairness.name, n_a, n_b, n_c)
    )
  ...
  test("Test byNameOrNull: TlaTempOper") {
    ...
    val leadsToBuildBad2 =
      bd.byNameOrNull(TlaTempOper.leadsTo.name, n_a, n_b, n_c)
    ...
    val guaranteesBuildBad2 =
      bd.byNameOrNull(TlaTempOper.guarantees.name, n_a, n_b, n_c)

    assert(guaranteesBuildBad1 == NullEx)
    assert(guaranteesBuild == OperEx(TlaTempOper.guarantees, n_a, n_b))
    assert(guaranteesBuildBad2 == NullEx)

    val strongFairnessBuildBad1 =
      bd.byNameOrNull(TlaTempOper.strongFairness.name, n_a)
    val strongFairnessBuild =
      bd.byNameOrNull(TlaTempOper.strongFairness.name, n_a, n_b)
    val strongFairnessBuildBad2 =
      bd.byNameOrNull(TlaTempOper.strongFairness.name, n_a, n_b, n_c)

    assert(strongFairnessBuildBad1 == NullEx)
    assert(strongFairnessBuild == OperEx(TlaTempOper.strongFairness, n_a, n_b))
    assert(strongFairnessBuildBad2 == NullEx)

    val weakFairnessBuildBad1 =
      bd.byNameOrNull(TlaTempOper.weakFairness.name, n_a)
    val weakFairnessBuild =
      bd.byNameOrNull(TlaTempOper.weakFairness.name, n_a, n_b)
    val weakFairnessBuildBad2 =
      bd.byNameOrNull(TlaTempOper.weakFairness.name, n_a, n_b, n_c)
  ...
  test("Test direct methods: TlaArithOper") {
    ...
    assert(
      divBuild4 == OperEx(TlaArithOper.div, ValEx(TlaInt(1)), ValEx(TlaInt(2)))
    )
    ...
    assert(
      modBuild4 == OperEx(TlaArithOper.mod, ValEx(TlaInt(1)), ValEx(TlaInt(2)))
    )
    ...
    assert(
      expBuild4 == OperEx(TlaArithOper.exp, ValEx(TlaInt(1)), ValEx(TlaInt(2)))
    )
    ...
    assert(
      ltBuild4 == OperEx(TlaArithOper.lt, ValEx(TlaInt(1)), ValEx(TlaInt(2)))
    )
    ...
    assert(
      gtBuild4 == OperEx(TlaArithOper.gt, ValEx(TlaInt(1)), ValEx(TlaInt(2)))
    )
    ...
    assert(
      leBuild4 == OperEx(TlaArithOper.le, ValEx(TlaInt(1)), ValEx(TlaInt(2)))
    )
    ...
    assert(
      geBuild4 == OperEx(TlaArithOper.ge, ValEx(TlaInt(1)), ValEx(TlaInt(2)))
    )
  }
  ...
  test("Test byName: TlaArithOper") {
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.plus.name, n_a)
    )
    assert(plusBuild == OperEx(TlaArithOper.plus, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.plus.name, n_a, n_b, n_c)
    )

    val minusBuild = bd.byName(TlaArithOper.minus.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.minus.name, n_a)
    )
    assert(minusBuild == OperEx(TlaArithOper.minus, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.minus.name, n_a, n_b, n_c)
    )
    ...
    assert(uminusBuild == OperEx(TlaArithOper.uminus, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.uminus.name, n_a, n_b, n_c)
    )
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.lt.name, n_a, n_b, n_c)
    )
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.gt.name, n_a, n_b, n_c)
    )
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.le.name, n_a, n_b, n_c)
    )
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaArithOper.ge.name, n_a, n_b, n_c)
    )
  }
  ...
  test("Test byNameOrNull: TlaArithOper") {
    ...
    val uminusBuildBad2 =
      bd.byNameOrNull(TlaArithOper.uminus.name, n_a, n_b, n_c)
    ...
    val realDivBuildBad2 =
      bd.byNameOrNull(TlaArithOper.realDiv.name, n_a, n_b, n_c)
    ...
    val dotdotBuildBad2 =
      bd.byNameOrNull(TlaArithOper.dotdot.name, n_a, n_b, n_c)
  ...
  test("Test byName: TlaFiniteSetOper") {
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFiniteSetOper.cardinality.name)
    )
    assert(cardinalityBuild == OperEx(TlaFiniteSetOper.cardinality, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFiniteSetOper.cardinality.name, n_a, n_b, n_c)
    )

    val isFiniteSetBuild = bd.byName(TlaFiniteSetOper.isFiniteSet.name, n_a)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaFiniteSetOper.isFiniteSet.name)
    )
    assert(isFiniteSetBuild == OperEx(TlaFiniteSetOper.isFiniteSet, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFiniteSetOper.isFiniteSet.name, n_a, n_b, n_c)
    )
  }

  test("Test byNameOrNull: TlaFiniteSetOper") {
    val cardinalityBuildBad1 =
      bd.byNameOrNull(TlaFiniteSetOper.cardinality.name)
    val cardinalityBuild =
      bd.byNameOrNull(TlaFiniteSetOper.cardinality.name, n_a)
    val cardinalityBuildBad2 =
      bd.byNameOrNull(TlaFiniteSetOper.cardinality.name, n_a, n_b, n_c)

    assert(cardinalityBuildBad1 == NullEx)
    assert(cardinalityBuild == OperEx(TlaFiniteSetOper.cardinality, n_a))
    assert(cardinalityBuildBad2 == NullEx)

    val isFiniteSetBuildBad1 =
      bd.byNameOrNull(TlaFiniteSetOper.isFiniteSet.name)
    val isFiniteSetBuild =
      bd.byNameOrNull(TlaFiniteSetOper.isFiniteSet.name, n_a)
    val isFiniteSetBuildBad2 =
      bd.byNameOrNull(TlaFiniteSetOper.isFiniteSet.name, n_a, n_b, n_c)
  ...
  test("Test direct methods: TlaFunOper") {
    ...
    assertThrows[IllegalArgumentException](
      bd.except(n_a, n_b, n_c, n_d, n_e, n_f)
    )
    ...
    assertThrows[IllegalArgumentException](
      bd.funDef(n_a, n_b, n_c, n_d, n_e, n_f)
    )
  ...
  test("Test byName: TlaFunOper") {
    ...
    assert(appBuild == OperEx(TlaFunOper.app, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.app.name, n_a, n_b, n_c)
    )
    ...
    assert(domainBuild == OperEx(TlaFunOper.domain, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.domain.name, n_a, n_b, n_c)
    )
    ...
    assert(enumBuild1 == OperEx(TlaFunOper.enum, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.enum.name, n_a, n_b, n_c)
    )
    assert(enumBuild2 == OperEx(TlaFunOper.enum, n_a, n_b, n_c, n_d))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.enum.name, n_a, n_b, n_c, n_d, n_e)
    )

    val exceptBuild1 = bd.byName(TlaFunOper.except.name, n_a, n_b, n_c)
    val exceptBuild2 =
      bd.byName(TlaFunOper.except.name, n_a, n_b, n_c, n_d, n_e)

    assertThrows[IllegalArgumentException](bd.byName(TlaFunOper.except.name))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.except.name, n_a)
    )
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.except.name, n_a, n_b)
    )
    assert(exceptBuild1 == OperEx(TlaFunOper.except, n_a, n_b, n_c))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.except.name, n_a, n_b, n_c, n_d)
    )
    assert(exceptBuild2 == OperEx(TlaFunOper.except, n_a, n_b, n_c, n_d, n_e))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.except.name, n_a, n_b, n_c, n_d, n_e, n_f)
    )

    val funDefBuild1 = bd.byName(TlaFunOper.funDef.name, n_a, n_b, n_c)
    val funDefBuild2 =
      bd.byName(TlaFunOper.funDef.name, n_a, n_b, n_c, n_d, n_e)

    assertThrows[IllegalArgumentException](bd.byName(TlaFunOper.funDef.name))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.funDef.name, n_a)
    )
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.funDef.name, n_a, n_b)
    )
    assert(funDefBuild1 == OperEx(TlaFunOper.funDef, n_a, n_b, n_c))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.funDef.name, n_a, n_b, n_c, n_d)
    )
    assert(funDefBuild2 == OperEx(TlaFunOper.funDef, n_a, n_b, n_c, n_d, n_e))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaFunOper.funDef.name, n_a, n_b, n_c, n_d, n_e, n_f)
    )
  ...
  test("Test byNameOrNull: TlaFunOper") {
    ...
    val enumBuildBad2 =
      bd.byNameOrNull(TlaFunOper.enum.name, n_a, n_b, n_c, n_d, n_e)
    ...
    val exceptBuildBad2 =
      bd.byNameOrNull(TlaFunOper.except.name, n_a, n_b, n_c, n_d)
    val exceptBuild2 =
      bd.byNameOrNull(TlaFunOper.except.name, n_a, n_b, n_c, n_d, n_e)
    ...
    val funDefBuildBad2 =
      bd.byNameOrNull(TlaFunOper.funDef.name, n_a, n_b, n_c, n_d)
    val funDefBuild2 =
      bd.byNameOrNull(TlaFunOper.funDef.name, n_a, n_b, n_c, n_d, n_e)
  ...
  test("Test byName: TlaSeqOper") {
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSeqOper.append.name, n_a)
    )
    assert(appendBuild == OperEx(TlaSeqOper.append, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSeqOper.append.name, n_a, n_b, n_c)
    )

    val concatBuild = bd.byName(TlaSeqOper.concat.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSeqOper.concat.name, n_a)
    )
    assert(concatBuild == OperEx(TlaSeqOper.concat, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSeqOper.concat.name, n_a, n_b, n_c)
    )
    ...
    assert(headBuild == OperEx(TlaSeqOper.head, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSeqOper.head.name, n_a, n_b)
    )
    ...
    assert(tailBuild == OperEx(TlaSeqOper.tail, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSeqOper.tail.name, n_a, n_b)
    )
    ...
    assert(lenBuild == OperEx(TlaSeqOper.len, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSeqOper.len.name, n_a, n_b)
    )
  }
  ...
  test("Test byName: TlaSetOper") {
    ...
    assert(inBuild == OperEx(TlaSetOper.in, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.in.name, n_a, n_b, n_c)
    )

    val notinBuild = bd.byName(TlaSetOper.notin.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.notin.name, n_a)
    )
    assert(notinBuild == OperEx(TlaSetOper.notin, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.notin.name, n_a, n_b, n_c)
    )
    ...
    assert(capBuild == OperEx(TlaSetOper.cap, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.cap.name, n_a, n_b, n_c)
    )
    ...
    assert(cupBuild == OperEx(TlaSetOper.cup, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.cup.name, n_a, n_b, n_c)
    )
    ...
    assert(unionBuild == OperEx(TlaSetOper.union, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.union.name, n_a, n_b)
    )

    val filterBuild = bd.byName(TlaSetOper.filter.name, n_a, n_b, n_c)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.filter.name, n_a, n_b)
    )
    assert(filterBuild == OperEx(TlaSetOper.filter, n_a, n_b, n_c))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.filter.name, n_a, n_b, n_c, n_d)
    )

    val mapBuild1 = bd.byNameOrNull(TlaSetOper.map.name, n_a, n_b, n_c)
    val mapBuild2 =
      bd.byNameOrNull(TlaSetOper.map.name, n_a, n_b, n_c, n_d, n_e)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.map.name, n_a, n_b)
    )
    assert(mapBuild1 == OperEx(TlaSetOper.map, n_a, n_b, n_c))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.map.name, n_a, n_b, n_c, n_d)
    )
    assert(mapBuild2 == OperEx(TlaSetOper.map, n_a, n_b, n_c, n_d, n_e))
    ...
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.funSet.name, n_a)
    )
    assert(funSetBuild == OperEx(TlaSetOper.funSet, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.funSet.name, n_a, n_b, n_c)
    )
    ...
    assert(recSetBuild1 == OperEx(TlaSetOper.recSet))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.recSet.name, n_a)
    )
    assert(recSetBuild2 == OperEx(TlaSetOper.recSet, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.recSet.name, n_a, n_b, n_c)
    )
    ...
    assert(seqSetBuild == OperEx(TlaSetOper.seqSet, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.seqSet.name, n_a, n_b)
    )

    val subsetBuild = bd.byName(TlaSetOper.subsetProper.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.subsetProper.name, n_a)
    )
    assert(subsetBuild == OperEx(TlaSetOper.subsetProper, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.subsetProper.name, n_a, n_b, n_c)
    )

    val subseteqBuild = bd.byName(TlaSetOper.subseteq.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.subseteq.name, n_a)
    )
    assert(subseteqBuild == OperEx(TlaSetOper.subseteq, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.subseteq.name, n_a, n_b, n_c)
    )

    val supsetBuild = bd.byName(TlaSetOper.supsetProper.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.supsetProper.name, n_a)
    )
    assert(supsetBuild == OperEx(TlaSetOper.supsetProper, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.supsetProper.name, n_a, n_b, n_c)
    )

    val supseteqBuild = bd.byName(TlaSetOper.supseteq.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.supseteq.name, n_a)
    )
    assert(supseteqBuild == OperEx(TlaSetOper.supseteq, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.supseteq.name, n_a, n_b, n_c)
    )

    val setminusBuild = bd.byName(TlaSetOper.setminus.name, n_a, n_b)

    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.setminus.name, n_a)
    )
    assert(setminusBuild == OperEx(TlaSetOper.setminus, n_a, n_b))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.setminus.name, n_a, n_b, n_c)
    )
    ...
    assert(powSetBuild == OperEx(TlaSetOper.powerset, n_a))
    assertThrows[IllegalArgumentException](
      bd.byName(TlaSetOper.powerset.name, n_a, n_b)
    )
  ...
  test("Test byNameOrNull: TlaSetOper") {
    ...
    val filterBuildBad2 =
      bd.byNameOrNull(TlaSetOper.filter.name, n_a, n_b, n_c, n_d)
    ...
    val subsetBuildBad2 =
      bd.byNameOrNull(TlaSetOper.subsetProper.name, n_a, n_b, n_c)
    ...
    val subseteqBuildBad2 =
      bd.byNameOrNull(TlaSetOper.subseteq.name, n_a, n_b, n_c)
    ...
    val supsetBuildBad2 =
      bd.byNameOrNull(TlaSetOper.supsetProper.name, n_a, n_b, n_c)
    ...
    val supseteqBuildBad2 =
      bd.byNameOrNull(TlaSetOper.supseteq.name, n_a, n_b, n_c)
    ...
    val setminusBuildBad2 =
      bd.byNameOrNull(TlaSetOper.setminus.name, n_a, n_b, n_c)
