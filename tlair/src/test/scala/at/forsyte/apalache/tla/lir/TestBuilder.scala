...
class TestBuilder extends AnyFunSuite with TestingPredefs {
  ...
  test("Test byName: TlaFunOper") {
    ...
    assert(enumBuild2 == OperEx(TlaFunOper.rec, n_a, n_b, n_c, n_d))
    assertThrows[IllegalArgumentException](bd.byName(TlaFunOper.rec.name, n_a, n_b, n_c, n_d, n_e).untyped())
  ...
  test("Test byNameOrNull: TlaFunOper") {
    ...
    val enumBuild1 = bd.byNameOrNull(TlaFunOper.rec.name, n_a, n_b).untyped()
    val enumBuildBad1 = bd.byNameOrNull(TlaFunOper.rec.name, n_a, n_b, n_c).untyped()
