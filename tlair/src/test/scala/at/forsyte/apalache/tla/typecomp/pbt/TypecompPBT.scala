...
object GenTBuilderInstruction {
   ...
   *   - SparseTupT1 : TODO
   *
   * NOTE: The generated instruction will not have any shadowed names internally, but if a builder instance is supplied,
 ...
 *
 * When these implicits are in scope, ScalaCheck can perform type-based generation. Specifying the type of input to
 * [forAll] then means, 'for all x of type T'. For example, without having to explicitly invoke a generator, we can
 * write
 *
 * {{{
 * import at.forsyte.apalache.tla.typecomp.pbt.ArbTBuilderInstruction._
 * test("pairs of equal elements are equal") {
 *   implicit val builder = new ScopedBuilder()
 *   val prop =
 *     forAll { (a: TBuilderInstruction) =>
 *       build(builder.tuple(a, a)) == build(builder.tuple(a, a))
 *     }
 *
